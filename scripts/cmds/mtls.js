...
const MTLSUtils_1 = require("@cass-modules/MTLSUtils");
exports.meta = {
    name: "mtls",
...
exports.style = {
    title: {
        content: `${unispectra_1.UNISpectra.charm} MTLS V4 🪙`,
        line_bottom: "default",
...
const configs = [
    {
        ...
        async handler({ usersDB, input, output }, { spectralArgs }) {
            ...
            const amount = (0, ArielUtils_1.parseBet)(spectralArgs[0], userData.money);
            if ((0, MTLSUtils_1.isInvalidAm)(amount, userData.money)) {
                return output.reply(`📋 | The amount (first argument) must be a **valid numerical**, not lower than **1**, and **not higher** than your **balance.** (${(0, ArielUtils_1.formatCash)(userData.money, true)})`);
            }
            ...
            if (lendAmount > 0 && userData.lendTimestamp) {
                return output.reply(`📋 | You cannot lend right now. You already have a **valid lend** of ${(0, ArielUtils_1.formatCash)(lendAmount, true)}, please **retrieve** it first!`);
            }
            ...
            return output.reply(`💌 | Successfully lent ${(0, ArielUtils_1.formatCash)(amount, true)}\n\nYour new **balance** is: ${(0, ArielUtils_1.formatCash)(newBal, true)}`);
        },
    },
    {
        key: "retrieve",
        ...
        args: ["[force]"],
        icon: "📥",
        async handler({ usersDB, input, output, getInflationRate }, { spectralArgs }) {
            const userData = await usersDB.getItem(input.sid);
            ...
            const now = Date.now();
            const durationInSeconds = Math.max((now - userData.lendTimestamp) / 1000 - 60 * 60 * 1000, 0);
            const inflationRate = await getInflationRate();
            const interestNoInflation = lendAmount * (0.001 / 365) * durationInSeconds;
            const interest = Math.floor(Math.max(0, interestNoInflation - interestNoInflation * (inflationRate / 1000)));
            ...
            if (interestCapped < 1 && !isForce) {
                return output.reply(`📋 | You **cannot retrieve** this lent amount because the **capped interest** is too **LOW** (${(0, ArielUtils_1.formatCash)(interestCapped, true)}). You would **not earn** anything. Please wait or add a **force** argument.`);
            }
            await usersDB.setItem(input.sid, {
                money: newBal,
                lendTimestamp: null,
            ...
            return output.reply(`🎉 | Successfully retrieved ${(0, ArielUtils_1.formatCash)(totalAmount, true)}$. (***GAIN*** = ${(0, ArielUtils_1.formatCash)(interestCapped, true)})\n\nYour new balance is: ${(0, ArielUtils_1.formatCash)(newBal, true)}`);
        },
    },
    {
        key: "send",
        ...
        async handler({ usersDB, input, output, Inventory }, { spectralArgs }) {
            ...
            if (!recipient ||
                ...
                return output.reply(`❕ | Recipient **not found**. Ensure you are providing the correct user's **name** or user's **ID** as a first argument.`);
            }
            if (recipient.userID === input.sid) {
                return output.reply(`❕ | You cannot send money **to yourself**!`);
            }
            const amount = (0, ArielUtils_1.parseBet)(spectralArgs[1], userData.money);
            if ((0, MTLSUtils_1.isInvalidAm)(amount, userData.money)) {
                return output.reply(`📋 | The amount (second argument) must be a **valid numerical**, not lower than **1**, and **not higher** than your **balance.** (${(0, ArielUtils_1.formatCash)(userData.money, true)})`);
            }
            const newBal = Number(userData.money - amount);
            const reciBal = Number(recipient.money + amount);
            if (reciBal < recipient.money ||
                isNaN(reciBal) ||
                isNaN(newBal) ||
                isNaN(amount)) {
                console.log({
            ...
            return output.reply(`💥 | Successfully used **0** 🌑 to send ${(0, ArielUtils_1.formatCash)(amount, true)}$ to **${recipient.name ?? "Unregistered"}**\n\nRemaining **Shadow Coins**: ${(0, ArielUtils_1.formatCash)(inventory.getAmount("shadowCoin"), "🌑", true)}`);
        },
    },
    {
        key: "inspect",
        ...
        async handler({ usersDB, output }, { spectralArgs }) {
            ...
            if (!recipient ||
                (recipient?.name !== targTest && recipient?.userID !== targTest)) {
                return output.reply(`❕ | Target **not found**. Ensure you are providing the correct user's **name** or user's **ID** as a first argument.`);
            }
            const texts = [
                `👤 | **Name**: ${recipient.name}`,
                `🪙 | **Balance**: ${(0, ArielUtils_1.formatCash)(recipient.money, true)}`,
                `🎲 | **User ID**: ${recipient.userID}`,
                `📤 | **Lent Amount**: ${(0, ArielUtils_1.formatCash)(recipient.lendAmount ?? 0, true)}`,
                `⏳ | **Lent Since**: ${recipient.lendTimestamp
                    ? `${(0, unispectra_1.formatTime)(Date.now() - recipient.lendTimestamp)}`
                    : "No active lend."}`,
            ];
        ...
        async handler({ usersDB, output, input, globalDB }, { spectralArgs, key }) {
            ...
            if (!result.success) {
                if (result.error === "Mint limit reached") {
                    return output.reply(`📋 | Cannot create mint: You have reached the limit of **${MTLSUtils_1.MintManager.MINT_LIMIT}** mints.`);
                }
                if (result.error === "Mint already exists" && result.existingMint) {
                    return output.reply(`📋 | Mint already exists!\n\n${await (0, MTLSUtils_1.formatMint)(result.existingMint, usersDB)}`);
                }
                return output.wentWrong();
            }
            await mintManager.saveBy(globalDB);
            return output.reply(`🪙 | Successfully created!\n\n${await (0, MTLSUtils_1.formatMint)(mint, usersDB)}`);
        },
    },
    {
        key: "list",
        ...
        async handler({ usersDB, output, input, globalDB }, {}) {
            ...
            const mapped = (await Promise.all(userMints.map(async (i) => `${await (0, MTLSUtils_1.formatMint)(i, usersDB)}`))).join("\n\n");
    ...
    },
    {
        ...
        async handler({ usersDB, output, input, globalDB }, { spectralArgs }) {
            ...
            await mintManager.saveBy(globalDB);
            return output.reply(`🗑️ | **DELETED**\n\n${await (0, MTLSUtils_1.formatMint)(deletedMint, usersDB)}`);
        },
    },
    {
        key: "mint",
        ...
        async handler({ usersDB, output, input, globalDB }, { spectralArgs }) {
            ...
            if ((0, MTLSUtils_1.isInvalidAm)(amount, Infinity)) {
                return output.reply(`📋 | The amount must be a **valid number**, not less than **1**.`);
            }
            ...
            const updatedMint = { ...mint, copies: newCopies };
            ...
            const converted = (0, MTLSUtils_1.convertMintToCll)(updatedMint);
            if (!cll.has(KEY)) {
            ...
            await mintManager.saveBy(globalDB);
            return output.reply(`🪙 | Minted **${amount}** copies of **${mint.name}** [${mint.id}].\nTotal copies: **${newCopies}**\n\n${await (0, MTLSUtils_1.formatMint)(updatedMint, usersDB)}`);
        },
    },
    {
        key: "fund",
        ...
        async handler({ usersDB, output, input, globalDB }, { spectralArgs }) {
            ...
            if ((0, MTLSUtils_1.isInvalidAm)(amount, userData.money)) {
                return output.reply(`📋 | The amount must be a **valid number**, not less than **1**, and not more than your **balance** (${(0, ArielUtils_1.formatCash)(userData.money, true)}).`);
            }
            const mint = mintManager
                .getUserMints(input.sid)
                .find((i) => i.id === tokenId);
            ...
            const updatedMint = { ...mint, asset: newAsset };
            const success = mintManager.updateMint(input.sid, updatedMint);
            if (!success) {
                return output.wentWrong();
            }
            ...
            await mintManager.saveBy(globalDB);
            return output.reply(`💰 | Added ${(0, ArielUtils_1.formatCash)(amount, true)} to **${mint.name}** [${mint.id}].\nNew asset value: ${(0, ArielUtils_1.formatCash)(newAsset, true)}\nNew balance: ${(0, ArielUtils_1.formatCash)(newBal, true)}\n\n${await (0, MTLSUtils_1.formatMint)(updatedMint, usersDB)}`);
        },
    },
    {
        key: "burn",
        ...
        async handler({ usersDB, output, input, globalDB }, { spectralArgs }) {
            ...
            const KEY = `mtls_${tokenId}`;
            if (!cll.has(KEY)) {
                return output.reply(`📋 | You have no collectibles for **${mint.name}** [${tokenId}].`);
            }
            if (!cll.hasAmount(KEY, amount)) {
                return output.reply(`📋 | Not enough collectibles for **${mint.name}** [${tokenId}]. You have ${cll.getAmount(KEY)}.`);
            }
            ...
            if (actualSurrendered <= 0) {
                return output.reply(`📋 | Cannot surrender **${amount}** copies. At least **1** copy must remain (current: ${currentCopies}).`);
            }
            const updatedMint = { ...mint, copies: newCopies };
            const success = mintManager.updateMint(input.sid, updatedMint);
            if (!success) {
            ...
            await mintManager.saveBy(globalDB);
            return output.reply(`🗑️ | Surrendered **${actualSurrendered}** copies of **${mint.name}** [${mint.id}].\nRemaining copies: **${newCopies}**\n\n${await (0, MTLSUtils_1.formatMint)(updatedMint, usersDB)}`);
        },
    },
    {
        key: "top",
    ...
    },
    {
        ...
        async handler({ usersDB, output, input, globalDB }, { spectralArgs }) {
            ...
            const userData = await usersDB.getItem(input.sid);
            const tokenId = spectralArgs[0] ?? "";
            const amount = (0, ArielUtils_1.parseBet)(spectralArgs[1], Infinity);
            if (!tokenId) {
                return output.reply(`📋 | Please provide a valid **token ID**.`);
            }
            if ((0, MTLSUtils_1.isInvalidAm)(amount, Infinity)) {
                return output.reply(`📋 | The first argument must be an **ID**. The amount (second argument) must be a **valid number**, not less than **1**.`);
            }
            ...
            const cll = new InventoryEnhanced_1.Collectibles(userData.collectibles ?? []);
            const KEY = `mtls_${tokenId}`;
            const converted = (0, MTLSUtils_1.convertMintToCll)(mint);
            if (!cll.has(KEY)) {
                cll.register(KEY, converted);
            }
            const totalAssetIncrease = mint.asset * ((mint.copies + amount) / mint.copies - 1);
            ...
            if (newBal < 0) {
                return output.reply(`📋 | You do not have **enough balance** to pay ${(0, ArielUtils_1.formatCash)(Math.floor(totalAssetIncrease), true)}`);
            }
            const updatedMint = {
                ...mint,
            ...
            await mintManager.saveBy(globalDB);
            return output.reply(`🔄 | Converted ${(0, ArielUtils_1.formatCash)(Math.floor(totalAssetIncrease), true)} to **${amount}** copies of **${mint.name}** [${mint.id}].\nNew balance: ${(0, ArielUtils_1.formatCash)(newBal, true)}\n\n${await (0, MTLSUtils_1.formatMint)(updatedMint, usersDB)}`);
        },
    },
    {
        key: "cashify",
        ...
        async handler({ usersDB, output, input, globalDB }, { spectralArgs }) {
            ...
            if (actualConverted <= 0) {
                return output.reply(`📋 | Cannot convert **${amount}** copies. At least **1** copy must remain (current: ${currentCopies}).`);
            }
            const totalAssetDecrease = mint.asset * (1 - (mint.copies - actualConverted) / mint.copies);
            ...
            await mintManager.saveBy(globalDB);
            return output.reply(`💸 | Converted **${actualConverted}** copies of **${mint.name}** [${mint.id}] to ${(0, ArielUtils_1.formatCash)(convertedMoney, true)}.\nRemaining copies: **${newCopies}**\nNew balance: ${(0, ArielUtils_1.formatCash)(newBal, true)}\n\n${await (0, MTLSUtils_1.formatMint)(updatedMint, usersDB)}`);
        },
    },
    {
        key: "check",
        ...
        async handler({ output, globalDB, usersDB }, { spectralArgs }) {
            ...
            return output.reply(`🔍 | **Mint Details**\n\n${await (0, MTLSUtils_1.formatMint)(mint, usersDB)}`);
        },
    },
];
const home = new spectral_home_1.SpectralCMDHome({
    ...
    async home({ output, input, usersDB }, { itemList }) {
        ...
        return output.reply(`💌 | Hello **${cache.name}**! Welcome to ${cassidy_styler_1.FontSystem.applyFonts("MTLS", "double_struck")} (Minting Token and Lending Service). Please use one of our **services**:\n\n${itemList}\n\n📤 | **Lent Amount**: ${(0, ArielUtils_1.formatCash)(cache.lendAmount ?? 0, true)}\n⏳ | **Lent Since**: ${cache.lendTimestamp
            ? `${(0, unispectra_1.formatTime)(Date.now() - cache.lendTimestamp)}`
            : "No active lend."}`);
    },
}, configs);
exports.entry = (0, define_1.defineEntry)((ctx) => home.runInContext(ctx));
