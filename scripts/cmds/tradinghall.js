...
const pCy = ArielUtils_1.formatCash;
...
exports.entry = (0, define_1.defineEntry)({
    async sell({ input, output, money, Inventory, args }) {
        ...
        const key = args[0];
        const price = (0, ArielUtils_1.parseBet)(args[1], Infinity) || 0;
        const amount = (0, ArielUtils_1.parseBet)(args[2] ?? "1", inventory.getAmount(key)) || 1;
        if (!key || !price || !amount) {
            return output.reply(guide);
        }
        if (!inventory.has(key)) {
            return output.reply(`❌ You don't have **${key}** in your briefcase 🧰!\n\n${guide}`);
        }
        if (isNaN(amount) || amount <= 0 || inventory.getAmount(key) < amount) {
            return output.reply(`❌ The amount you entered is **invalid**.\n\n${guide}`);
        }
        ...
        if (existing && existing.price && existing.price !== price) {
            return output.reply(`🪙 The price of existing ${(0, BriefcaseAPI_1.listItem)(existing, tradeVentory.getAmount(existing.key))} was **${pCy(existing.price)}**, your prices must be **consistent**!`);
        }
        if (amount + tradeVentory.getAll().length > invLimit) {
            return output.reply(`🧰 You can only **hold** up to ${invLimit} **items** in your trading hall, you currently have **${tradeVentory.getAll().length}** items in the **hall**.`);
        }
        ...
        return output.reply(`✅ Added **${amount}** item(s) for **${pCy(price)}** each to the trading hall!\n\n${itemsInv
            .toUnique((i) => i.key)
            .map((item) => (0, BriefcaseAPI_1.listItem)(item, itemsInv.getAmount(item.key)))
            .join("\n")}`);
    },
    async buy({ input, output, money, Inventory, args, Slicer, TagParser, CassExpress, }) {
            ...
            if (data) {
                for (const { tag, value } of data) {
                    ...
                    mappedTrades = mappedTrades.filter((trades) => {
                        return trades.some((item) => String(item[tag]).toLowerCase().includes(value.toLowerCase()));
                    });
                }
            }
        }
        ...
        for (const trade of slicer.getPage(args[0])) {
            i++;
            const num = Object.keys(preservedIndex).find((key) => preservedIndex[key] === trade.userID);
            ...
            for (const item of tradeX.toUnique((i) => i.key).reverse()) {
                ...
                const emoji1 = userMoney >= item.price
                    ? invAmount || boxAmount
                        ? "✅"
                        : "💰"
                    : "❌";
                result += `${(0, BriefcaseAPI_1.listItem)(item, tradeX.getAmount(item.key))} ${emoji1}\n- **${pCy(item.price)} each**${invAmount ? ` 🧰 **x${invAmount}**` : ""}${boxAmount ? ` 🗃️ **x${boxAmount}**` : ""}\n${unispectra_1.UNIRedux.charm} ${(0, unispectra_1.limitString)(item.flavorText, 100)}\n\n`;
                existedKeys.push(item.key);
            }
            result += `${unispectra_1.UNIRedux.standardLine}\n`;
        }
...
**${pCy(userMoney)}** 🧰 **${inventory.getAll().length}/${invLimit}**`;
        ...
        async function handleBuy(r) {
            ...
            if (userID === input.senderID) {
                return output.reply(`❌ You can't buy your own items! Consider **cancelling** instead.`);
            }
            ...
            amount = (0, ArielUtils_1.parseBet)(amount, tradeVentory.getAmount(key));
            if (!key) {
                return output.reply(`💌 Please enter a **key** to buy, haven't you read the guide?`);
            }
            if (!tradeVentory.has(key)) {
                return output.reply(`❌ **${trader}** doesn't have "${key}" in their trading hall!`);
            }
            ...
            return output.reply(`✅ You bought **${bought.filter((i) => !i.error).length}** item(s) from 👤 **${trader}**!\n\n${boughtInv
                .toUnique()
                .map((i) => `${(0, BriefcaseAPI_1.listItem)(i, boughtInv.getAmount(i.key))} - **${pCy(i.price * boughtInv.getAmount(i.key))}** ${i.error ? `\n❌ ${i.error}\n` : ""}`)
    ...
    async list({ input, output, args, money, Inventory }) {
        ...
        let result = `${unispectra_1.UNIRedux.arrow} 🪙👤 **${name}'s** Trading Hall **(${tradeVentory.size()}/${invLimit})**\n\n`;
        for (const item of tradeVentory.toUnique()) {
            result += `${(0, BriefcaseAPI_1.listItem)(item, tradeVentory.count(item.key))} - **${pCy(item.price)}**\n${unispectra_1.UNIRedux.charm} ${(0, unispectra_1.limitString)(item.flavorText, 100)}\n\n`;
        }
    ...
    async cancel({ input, output, args, money, Inventory }) {
        ...
        const inventory = new Inventory(userData.inventory ?? []);
        const key = args[0];
        const amount = (0, ArielUtils_1.parseBet)(args[1] ?? "1", tradeVentory.getAmount(key)) || 0;
        if (!key || !amount) {
            return output.reply(`💌 Please enter a **key** and **amount**.`);
        }
        if (!tradeVentory.hasAmount(key, amount)) {
            return output.reply(`❌ The amount of "${key}" you want to cancel is **invalid**, you currently have ${tradeVentory.getAmount(key)} of it.`);
        }
        if (inventory.getAll().length + amount > invLimit) {
            return output.reply(`🧰 You can only **hold** up to ${invLimit} **items** in your briefcase, you currently have **${inventory.getAll().length}/${invLimit}** items 🧰.`);
        }
        ...
        return output.reply(`✅ You canceled **${amount}** item(s)!\n\n${itemsInv
            .toUnique((i) => i.key)
            .map((item) => (0, BriefcaseAPI_1.listItem)(item, itemsInv.getAmount(item.key)))
            .join("\n")}`);
    },
