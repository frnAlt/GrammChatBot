...
const pet_fight_1 = require("../plugins/pet-fight");
...
async function entry({ input, output, money, args, prefix, ctx, globalDB, }) {
    const userData = await money.getCache(input.senderID);
    const { inventory, gearsData, collectibles } = getDatas(userData);
    function getDatas(data) {
        const inventory = new InventoryEnhanced_1.Inventory(data.inventory || []);
    ...
    const ranks = [
        {
            name: cassidy_styler_1.FontSystem.applyFonts("NOVICE", "double_struck"),
            keySuffix: "NOVICE",
            statMultiplier: 1.1,
            copiesRequired: 2,
            cost: {
        ...
        {
            name: cassidy_styler_1.FontSystem.applyFonts("STARLIT", "double_struck"),
            keySuffix: "STARLIT",
            statMultiplier: 1.15,
            copiesRequired: 2,
            cost: {
        ...
        {
            name: cassidy_styler_1.FontSystem.applyFonts("LUNAR", "double_struck"),
            keySuffix: "LUNAR",
            statMultiplier: 1.2,
            copiesRequired: 3,
            cost: {
        ...
        {
            name: cassidy_styler_1.FontSystem.applyFonts("SOLAR", "double_struck"),
            keySuffix: "SOLAR",
            statMultiplier: 1.25,
            copiesRequired: 3,
            cost: {
        ...
        {
            name: cassidy_styler_1.FontSystem.applyFonts("ASTRAL", "double_struck"),
            keySuffix: "ASTRAL",
        ...
        {
            name: cassidy_styler_1.FontSystem.applyFonts("CRYSTAL", "double_struck"),
            keySuffix: "CRYSTAL",
        ...
        {
            name: cassidy_styler_1.FontSystem.applyFonts("NEBULA", "double_struck"),
            keySuffix: "NEBULA",
        ...
        {
            name: cassidy_styler_1.FontSystem.applyFonts("GALAXY", "double_struck"),
            keySuffix: "GALAXY",
        ...
        {
            name: cassidy_styler_1.FontSystem.applyFonts("STARDUST", "double_struck"),
            keySuffix: "STARDUST",
        ...
        {
            name: cassidy_styler_1.FontSystem.applyFonts("ETERNAL", "double_struck"),
            keySuffix: "ETERNAL",
        ...
        {
            name: cassidy_styler_1.FontSystem.applyFonts("VOID", "double_struck"),
            keySuffix: "VOID",
            statMultiplier: 4.2,
            copiesRequired: 10,
            cost: {
        ...
        {
            name: cassidy_styler_1.FontSystem.applyFonts("COSMIC", "double_struck"),
            keySuffix: "COSMIC",
            statMultiplier: 4.5,
            copiesRequired: 11,
            cost: {
        ...
        {
            name: cassidy_styler_1.FontSystem.applyFonts("INFINITE", "double_struck"),
            keySuffix: "INFINITE",
            statMultiplier: 4.8,
            copiesRequired: 12,
            cost: {
        ...
        {
            name: cassidy_styler_1.FontSystem.applyFonts("DIVINE", "double_struck"),
            keySuffix: "DIVINE",
            statMultiplier: 5.0,
            copiesRequired: 13,
            cost: {
        ...
        {
            name: cassidy_styler_1.FontSystem.applyFonts("AWAKENED", "double_struck"),
            keySuffix: "AWAKENED",
            statMultiplier: 5.5,
            copiesRequired: 15,
            cost: {
    ...
    const isUpgraderEnabled = (await globalDB.getCache("global_settings"))?.upgraderEnabled ?? false;
    if (!isUpgraderEnabled && !input.isAdmin) {
        return output.reply("❌ The upgrader is currently disabled. Please contact an admin.");
    }
    const action = args[0]?.toLowerCase() === "u" ? "upgrade" : args[0]?.toLowerCase();
    ...
    const configs = [
        {
            ...
            async handler() {
                if (!itemKey) {
                    return output.reply("❌ Please specify an item key to upgrade (e.g., +upgrader-u hakurousTanto).");
                }
                const item = inventory.getOne(itemKey);
                if (!item || (item.type !== "armor" && item.type !== "weapon")) {
                    return output.reply(`❌ Item "${itemKey}" is not a valid armor or weapon.`);
                }
                const currentRankIndex = ranks.findIndex((rank) => item.name.includes(rank.name));
                const nextRankIndex = currentRankIndex === -1 ? 0 : currentRankIndex + 1;
                if (nextRankIndex >= ranks.length) {
                    return output.reply(`❌ ${item.icon} **${item.name}** is already at the maximum rank!`);
                }
                ...
                if (itemCopies < copiesRequired) {
                    const nextName = currentRankIndex === -1
                        ? `${item.name} ${nextRank.name}`
                        : item.name.replace(ranks[currentRankIndex].name, nextRank.name);
                    return output.reply(`❌ You don't have enough copies of **${item.name}** to upgrade to **${nextName}**. ` +
                        `Required: ${copiesRequired} copies (You have: ${itemCopies}).`);
                }
                if ((userData.battlePoints || 0) < cost.battlePoints ||
                    ...
                    const nextName = currentRankIndex === -1
                        ? `${item.name} ${nextRank.name}`
                        : item.name.replace(ranks[currentRankIndex].name, nextRank.name);
                    return output.reply(`❌ You don't have enough resources to upgrade **${item.name}** to **${nextName}**. ` +
                        `Required:\n` +
                        `${unispectra_1.UNIRedux.arrowFromT} ${copiesRequired} copies of **${item.name}**\n` +
                        `${unispectra_1.UNIRedux.arrowFromT} ⚔️ Battle Points: ${cost.battlePoints}\n` +
                        `${unispectra_1.UNIRedux.arrowFromT} 💎 Stellar Gems: ${cost.stellarGems}\n` +
                        `${unispectra_1.UNIRedux.arrowFromT} 🔮 Intertwined Fate: ${cost.intertwinedFate}\n` +
                        `${unispectra_1.UNIRedux.arrowFromT} 💎 Daily Gems: ${cost.dailyGems}\n` +
                        `${unispectra_1.UNIRedux.arrowFromT} 💵 Money: $${cost.money.toLocaleString()}`);
                }
                ...
                let baseName, baseKey, itemType, newName;
                if (currentRankIndex === -1) {
                    ...
                    itemType = item.name
                        .split(" ")[item.name.split(" ").length - 2].toLowerCase();
                    baseKey = itemKey.replace(ranks[currentRankIndex].keySuffix, "");
                    newName = `${baseName} ${itemType.charAt(0).toUpperCase() + itemType.slice(1)} ${nextRank.name}`;
                }
                ...
                const newMagic = Math.floor(currentMagic * nextRank.statMultiplier) || 1;
                const newItem = {
                    ...item,
                    name: newName,
                    key: newKey,
                ...
                return output.reply(`✅ Successfully upgraded **${baseName} ${itemType.charAt(0).toUpperCase() + itemType.slice(1)}** to **${newName}**!\n` +
                    `New Key: ${newKey}\n` +
                    `New Stats:\n` +
                    `${unispectra_1.UNIRedux.arrowFromT} ATK: ${newItem.atk}\n` +
                    `${unispectra_1.UNIRedux.arrowFromT} DEF: ${newItem.def}\n` +
                    `${unispectra_1.UNIRedux.arrowFromT} MAGIC: ${newItem.magic}\n` +
                    `Cost:\n` +
                    `${unispectra_1.UNIRedux.arrowFromT} ${copiesRequired} copies of **${item.name}**\n` +
                    `${unispectra_1.UNIRedux.arrowFromT} ⚔️ Battle Points: ${cost.battlePoints}\n` +
                    `${unispectra_1.UNIRedux.arrowFromT} 💎 Stellar Gems: ${cost.stellarGems}\n` +
        ...
        },
        {
            ...
            async handler() {
                ...
                if (!state || (state !== "on" && state !== "off")) {
                    return output.reply(`❌ Please specify 'on' or 'off'. Usage: \`${prefix}upgrader toggle <on|off>\``);
                }
                ...
                return output.reply(`✅ Upgrader is now **${state === "on" ? "ENABLED" : "DISABLED"}**.`);
            },
        },
        {
            key: "list",
            ...
            async handler() {
                ...
                if (upgradableItems.length === 0) {
                    return output.reply("❌ You have no upgradable items (armor or weapon).");
                }
                ...
                for (const item of upgradableItems) {
                    ...
                    if (currentRankIndex === -1) {
                        ...
                        nextName = `${baseName} ${itemType.charAt(0).toUpperCase() + itemType.slice(1)} ${nextRank.name}`;
                    }
    ...
    const home = new spectral_home_1.SpectralCMDHome({
        ...
        async home({ output, input, globalDB, money }, { itemList }) {
            ...
            const upgraderStatus = (await globalDB.getCache("global_settings"))?.upgraderEnabled ??
                false;
            return output.reply(`⚒️ | Hello **${cache.name || "Unregistered"}**! Welcome to ${cassidy_styler_1.FontSystem.applyFonts("Gear Upgrader", "double_struck")}.\n\n` +
                `🔧 | **Status**: ${upgraderStatus ? "Enabled" : "Disabled (Admin Access Only)"}\n` +
