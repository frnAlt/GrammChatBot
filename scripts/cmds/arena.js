...
const output_1 = require("@cass-plugins/output");
...
exports.style = {
    title: `⚔️ Arena ${cassidy_styler_1.FontSystem.applyFonts("EX", "double_struck")}`,
    titleFont: "bold",
    contentFont: "fancy",
    lineDeco: "altar",
};
...
const MAX_TURNS = 100;
async function generateAIPet(money, player1Pet, requestedName = "") {
    ...
    loop1: for (const user of Object.values(allUsers)) {
        ...
        for (const petData of petsData.getAll()) {
            ...
            const isDisqualified = pet.petIcon === playerPetIcon &&
...
function generateAIMove(gameState, activePet, targetPet, petStats, prevMove) {
    ...
    const healEfficiency = Math.max(0.5, 1 - petStats.healsPerformed * HEAL_DIMINISHING_FACTOR);
    const attackBoostEfficiency = Math.max(0.5, 1 - petStats.attackBoosts * 0.25);
    const defenseBoostEfficiency = Math.max(0.5, 1 - petStats.defenseBoosts * 0.25);
    ...
    const opponentStats = gameState.player1Pet && gameState.player2Pet
        ? gameState.activePlayer === 2
            ? statMap.get(`${gameState.player1Author}_${gameState.player1Pet.OgpetData.key}`)
            : statMap.get(`${gameState.player2Author}_${gameState.player2Pet.OgpetData.key}`)
        : null;
    ...
    const calculateAverageAttack = (attacker, defenderDF, attackStat) => {
        let totalDamage = 0;
        for (let i = 0; i < 20; i++) {
    ...
    const calculateBashDamage = () => {
        return calculateAverageAttack(activePet, targetPet.DF);
    };
    const calculateHexSmashDamage = () => {
        const meanStat = Math.min((activePet.ATK + activePet.MAGIC) / 2, activePet.ATK * 3);
        return Math.round(calculateAverageAttack(activePet, targetPet.DF, meanStat) * 1.5);
    };
    const calculateFluxStrikeDamage = () => {
        const damageFactor = Math.max(0.5, 1 - petStats.totalDamageDealt / (targetPet.maxHP * 2));
        const fluxMultiplier = 1 + 0.5 * (targetPet.HP / targetPet.maxHP) * damageFactor;
        return Math.round(activePet.ATK * fluxMultiplier - targetPet.DF / 5);
    };
    const calculateChaosBoltDamage = () => {
        const statFactor = Math.min((activePet.ATK + activePet.MAGIC) / statThreshold, 1);
        const effectiveStat = Math.max(activePet.ATK, activePet.MAGIC / 2);
        let damage = calculateAverageAttack(activePet, targetPet.DF, effectiveStat) *
            statFactor;
        const chaosChance = Math.min(((activePet.ATK + activePet.MAGIC) / (targetPet.DF || 1)) * 0.25, 0.35) *
    ...
    const calculateEquilibriumEffect = () => {
        const eqFactor = Math.min(1 + petStats.totalDamageTaken / (activePet.maxHP * 2), 2.2);
        ...
        const damage = Math.round(calculateAverageAttack(activePet, targetPet.DF, (activePet.ATK + activePet.MAGIC) / 2) *
            (hpDiff / 100) *
            eqFactor *
            attackFactor);
        const heal = Math.round(((activePet.DF + activePet.MAGIC) / 4) *
            (hpDiff / 100) *
            eqFactor *
            defenseFactor +
    ...
    const calculateVitalSurgeHeal = () => {
        const healFactor = Math.min(1.6, 1 + (1 - petStats.healsPerformed * HEAL_DIMINISHING_FACTOR));
        const surgeHeal = Math.round(activePet.MAGIC * (1 + activePet.HP / activePet.maxHP) * 0.6 * healFactor);
        return Math.min(surgeHeal, activePet.maxHP - activePet.HP);
    };
    const calculateGuardPulseBoost = () => {
        const guardFactor = Math.max(0.5, 1 - petStats.defenseBoosts * 0.3);
        return Math.round(activePet.DF * (1 - activePet.HP / activePet.maxHP) * 1.8 * guardFactor);
    };
    const calculateStatSyncBoost = () => {
        const syncFactor = Math.max(0.5, 1 - petStats.attackBoosts * 0.3);
        return Math.round(Math.max(0, Math.min((activePet.DF + 1) *
            (targetPet.DF / (activePet.DF || 1)) *
            0.5 *
            syncFactor, activePet.level * 2.5)));
    };
    const assessDodgeRisk = (move) => {
        if (prevMove === move) {
            return (DODGE_RISK_MODIFIERS[move] ||
                DODGE_RISK_MODIFIERS.default);
        }
        return DODGE_RISK_BASE;
    };
    const assessCriticalSituation = () => {
        return (currentHPPercent <= CRITICAL_HP_THRESHOLD &&
            activePet.HP < activePet.maxHP);
    };
    const assessUrgentSituation = () => {
        return (currentHPPercent <= URGENT_HP_THRESHOLD && activePet.HP < activePet.maxHP);
    };
    const assessEndgamePressure = () => {
        if (isEndgame) {
    ...
    const predictOpponentMove = () => {
        ...
        if (["bash", "hexsmash", "fluxstrike", "chaosbolt"].includes(lastMove) &&
            Math.random() < 0.75) {
            return lastMove;
        }
        ...
        if (lastMove === "vitalsurge" && Math.random() < 0.8) {
            return ["bash", "hexsmash", "fluxstrike", "chaosbolt"][Math.floor(Math.random() * 4)];
        }
    ...
    if (isEndgame && targetHPPercent < 25)
        bashScore *= 1.6;
    if (isEarlyGame && attackAdvantage < 0.9)
        bashScore *= 0.7;
    if (opponentAttackBoosts > 1)
        bashScore *= 0.8;
    if (shouldPrioritizeStatSync || isLowDefense)
        bashScore *= 0.5;
    if (currentMood === "aggressive")
        bashScore *= 1.2;
    if (currentMood === "defensive")
        bashScore *= 0.8;
    if (moveHistory.player === "guardpulse")
        bashScore *= 1.2;
    if (moveHistory.player === "statsync")
        bashScore *= 0.8;
    moveScores.push({
        move: "bash",
        score: bashScore,
    ...
    let hexSmashScore = (hexSmashDamage *
        (1 - hexSmashDodgeRisk) *
        (attackAdvantage + magicAdvantage)) /
        2;
    if (magicAdvantage > 1.3)
        hexSmashScore *= 1.4;
    if (damageDealtRatio > 0.6)
        hexSmashScore *= 0.85;
    if (opponentDefenseBoosts > 1)
        hexSmashScore *= 1.2;
    if (shouldPrioritizeStatSync || isLowDefense)
        hexSmashScore *= 0.5;
    if (currentMood === "aggressive")
    ...
    let fluxStrikeScore = fluxStrikeDamage *
        (1 - fluxStrikeDodgeRisk) *
        attackAdvantage *
        (targetHPPercent / 100);
    if (targetHPPercent > 60)
        fluxStrikeScore *= 1.3;
    if (isEndgame && targetHPPercent < 35)
        fluxStrikeScore *= 1.5;
    if (opponentAttackBoosts > 2)
        fluxStrikeScore *= 0.9;
    if (shouldPrioritizeStatSync || isLowDefense)
        fluxStrikeScore *= 0.5;
    if (currentMood === "aggressive")
    ...
    let chaosBoltScore = chaosBoltDamage *
        (1 - chaosBoltDodgeRisk) *
        Math.max(attackAdvantage, magicAdvantage / 2);
    const chaosCriticalChance = Math.min(((activePet.ATK + activePet.MAGIC) / (targetPet.DF || 1)) * 0.25, 0.35);
    chaosBoltScore *= 1 + chaosCriticalChance * 0.6;
    if (isEarlyGame && levelDifference < -2)
        chaosBoltScore *= 0.6;
    if (isEndgame && targetHPPercent < 20)
        chaosBoltScore *= 1.8;
    if (opponentDefenseBoosts > 2)
        chaosBoltScore *= 1.3;
    if (shouldPrioritizeStatSync || isLowDefense)
        chaosBoltScore *= 0.5;
    if (currentMood === "aggressive")
    ...
    let vitalSurgeScore = vitalSurgeHeal * healEfficiency * (1 - currentHPPercent / 100);
    if (assessCriticalSituation())
        vitalSurgeScore *= 2.5;
    if (assessUrgentSituation())
        vitalSurgeScore *= 3.0;
    if (petStats.healsPerformed >= 4)
        vitalSurgeScore *= 0.5;
    if (magicAdvantage < 0.7)
        vitalSurgeScore *= 0.7;
    if (opponentDamageDealtRatio > 0.5)
        vitalSurgeScore *= 1.4;
    ...
    if (currentMood === "aggressive")
        vitalSurgeScore *= 0.8;
    if (moveHistory.player === "vitalsurge")
        vitalSurgeScore *= 0.8;
    if (["bash", "hexsmash", "fluxstrike", "chaosbolt"].includes(moveHistory.player))
        vitalSurgeScore *= 1.2;
    moveScores.push({
        move: "vitalsurge",
    ...
    let guardPulseScore = guardPulseBoost * defenseBoostEfficiency * (1 - defenseDisadvantage);
    if (petStats.defenseBoosts >= DEFENSE_BOOST_CAP)
        guardPulseScore = 0;
    if (damageTakenRatio > 0.6)
        guardPulseScore *= 1.6;
    if (isEarlyGame && defenseDisadvantage < 0.9)
        guardPulseScore *= 1.1;
    if (opponentAttackBoosts >= 2)
        guardPulseScore *= 1.5;
    if (isLowDefense && petStats.defenseBoosts < DEFENSE_BOOST_CAP)
        guardPulseScore *= 1.1;
    if (opponentDamageDealtRatio > 0.4)
        guardPulseScore *= 1.1;
    if (currentMood === "defensive")
        guardPulseScore *= 1.1;
    if (currentMood === "aggressive")
    ...
    if (petStats.attackBoosts >= ATTACK_BOOST_CAP)
        statSyncScore = 0;
    if (attackAdvantage < 0.9)
        statSyncScore *= 1.4;
    if (isEndgame)
        statSyncScore *= 0.7;
    if (opponentDefenseBoosts >= 2)
        statSyncScore *= 1.1;
    if (opponentDamageTakenRatio < 0.3)
        statSyncScore *= 1.1;
    ...
    let equilibriumScore = (equilibriumEffect.damage + equilibriumEffect.heal) * (hpDifference / 100);
    if (hpDifference <= 0)
        equilibriumScore = 0;
    if (hpDifference > HP_DIFF_THRESHOLD)
        equilibriumScore *= 1.6;
    ...
    if (["bash", "hexsmash", "fluxstrike", "chaosbolt"].includes(predictedOpponentMove)) {
    ...
    if (predictedOpponentMove === "guardpulse" &&
    ...
    if (predictedOpponentMove === "statsync" &&
        petStats.defenseBoosts < DEFENSE_BOOST_CAP) {
        moveScores.forEach((move) => {
            if (move.move === "guardpulse") {
                move.score *= 1.1;
    ...
    const assessMoveRisk = (move) => {
        ...
        if (move.boostType) {
            ...
            if (move.boostType === "attack" && opponentDefenseBoosts > 1)
                risk += 0.1;
            if (move.boostType === "defense" && opponentAttackBoosts > 1)
                risk -= 0.1;
        }
        return Math.max(-0.5, Math.min(0.5, risk));
    };
    const moodModifiers = {
        aggressive: (move) => move.expectedDamage ? 1.2 : move.boostType === "attack" ? 1.1 : 0.8,
        defensive: (move) => move.expectedHeal || move.move === "guardpulse"
            ? 1.2
...
async function entry({ input, output, money, ctx, prefix, commandName, getInflationRate, }) {
    ...
    if (petsData.getAll().length < 1)
        return output.replyStyled(`You need at least one pet.`, exports.style);
    const player1PetName = input.arguments.at(0);
    if (!player1PetName)
        return output.replyStyled(`Specify one pet name.`, exports.style);
    ...
    if (gameState.isAIMode) {
        const { pet: aiPet, author: aiAuthor } = await generateAIPet(money, player1Pet, input.arguments[1] !== "--ai" ? input.arguments[1] : "");
        ...
        gameState.flavorCache = `Arena battle begins! ${gameState.activePlayer === 1 ? player1Data.name : "AI Opponent"} goes first.`;
    ...
    const { pet: aiPetBefore } = await generateAIPet(money, player1Pet, input.arguments[1] !== "--ai" ? input.arguments[1] : "");
    const infoBegin = await output.replyStyled(`⚔️ **Arena Challenge**:\n${player1Data.name} selected:\n\n${player1Pet.getPlayerUI({
        showStats: true,
        hideHP: true,
    })}\n\nReply with one pet name to join.\n\n🔎 Don't have actual opponents, or you want to win 💎 gems? Try --ai now! Just put --ai at the end.\n***EXAMPLE*** ${prefix}${commandName} ${input.arguments[0]} --ai\n***EXAMPLE w/ OPPONENT*** ${prefix}${commandName} ${input.arguments[0]} ${aiPetBefore?.petName ?? "<pet_name>"} --ai`, exports.style);
    const startHandler = async (ctx) => {
        if (isDefeat || ctx.input.senderID === input.senderID) {
            await ctx.output.replyStyled(`❌ | A different player must join.`, exports.style);
            return;
        }
        const player2Data = await ctx.money.getItem(ctx.input.senderID);
        const { petsData: player2PetsData, playersMap: player2PlayersMap } = getInfos(player2Data);
        if (player2PetsData.getAll().length < 1) {
            await ctx.output.replyStyled(`❌ | You need one pet to join.`, exports.style);
            return;
        }
        ...
        if (!player2PetData) {
            await ctx.output.replyStyled(`❌ | Pet "${player2PetName}" not found.`, exports.style);
            return;
        }
        const player2Pet = player2PlayersMap.get(player2PetData.key);
        if (!player2Pet) {
            await ctx.output.replyStyled(`❌ | Error loading pet "${player2PetName}".`, exports.style);
            return;
        }
        ...
        gameState.flavorCache = `Arena battle begins! ${gameState.activePlayer === 1 ? player1Data.name : player2Data.name} goes first.`;
        statMap.set(`${gameState.player1Author}_${player1Pet.OgpetData.key}`, {
            totalDamageDealt: 0,
            totalDamageTaken: 0,
            mercyContributed: 0,
    ...
    async function displayPetSelection(ctx) {
        if (!gameState || !gameState.player1Pet || !gameState.player2Pet)
            return;
        const activePet = gameState.activePlayer === 1
            ? gameState.player1Pet
            : gameState.player2Pet;
        const opponentPet = gameState.activePlayer === 1
            ? gameState.player2Pet
            : gameState.player1Pet;
        const result = `${cassidy_styler_1.UNIRedux.charm} ${gameState.flavorCache}\n\n${activePet.getPlayerUI({
            selectionOptions: petSchema,
            turn: true,
            showStats: true,
        })}\n\n**Opponent**\n${opponentPet.getPlayerUI({
            showStats: true,
        })}\n\n⚠️ **Remaining turns before ending:  ${MAX_TURNS - (gameState.turnCount + 1)}**\n\n***Reply with one option (word only)***`;
        if (gameState.isAIMode && gameState.activePlayer === 2) {
            const aiMove = generateAIMove(gameState, gameState.player2Pet, gameState.player1Pet, statMap.get(`${gameState.player2Author}_${gameState.player2Pet.OgpetData.key}`), gameState.prevMove2);
            await handleArenaTurn(ctx, new output_1.OutputResult(ctx, {
                messageID: "dummy",
                threadID: "dummy",
                senderID: "dummy",
                timestamp: Date.now(),
            ...
            const newInfo = await ctx.output.replyStyled(result, exports.style);
            newInfo.atReply(async (turnCtx) => await handlePlayerTurn(turnCtx, newInfo));
        }
    }
    async function handlePlayerTurn(ctx, info, extraRes = "") {
        if (isDefeat ||
            !gameState ||
            !gameState.player1Pet ||
            !gameState.player2Pet) {
        ...
        if (isDefeat ||
            ...
            await ctx.output.replyStyled(`❌ | It's ${gameState?.activePlayer === 1 ? player1Data.name : player2Data.name}'s turn.`, exports.style);
            return;
        }
        const turn = ctx.input.body.toLowerCase().trim();
        await handleArenaTurn(ctx, info, turn);
    }
    async function handleArenaTurn(ctx, info, turn, extraAIRes = "") {
        if (!gameState || !gameState.player1Pet || !gameState.player2Pet)
            return;
        ...
        const activePet = gameState.activePlayer === 1
            ? gameState.player1Pet
            : gameState.player2Pet;
        const targetPet = gameState.activePlayer === 1
            ? gameState.player2Pet
            : gameState.player1Pet;
        const petStats = statMap.get(`${gameState.activePlayer === 1
            ? gameState.player1Author
            : gameState.player2Author}_${activePet.OgpetData.key}`);
        const opponentStats = statMap.get(`${gameState.activePlayer === 1
            ? gameState.player2Author
            : gameState.player1Author}_${targetPet.OgpetData.key}`);
        const prevMove = gameState.activePlayer === 1 ? gameState.prevMove1 : gameState.prevMove2;
        let flavorText = "";
        let dodgeChance = Math.random();
        if (activePet.isDown()) {
            await handleDefeat(ctx, info, gameState.activePlayer === 1 ? 2 : 1, false, extraAIRes);
            return;
        }
        if (turn === "generate") {
            turn = generateAIMove(gameState, activePet, targetPet, petStats, prevMove);
        }
        const turnCTX = {
            activePet,
            opponentStats,
        ...
        switch (turn) {
            case "cheat":
                if (ctx.input.isAdmin) {
                    const damage = targetPet.maxHP - activePet.calculateAttack(targetPet.DF);
                    targetPet.HP -= damage;
                    petStats.totalDamageDealt += damage;
                    opponentStats.totalDamageTaken += damage;
                    flavorText = `${cassidy_styler_1.UNIRedux.charm} ${activePet.petIcon} **${activePet.petName}** cheated! Dealt **${damage}** damage.\n${targetPet.getPlayerUI({})}`;
                }
                else {
                    flavorText = `${cassidy_styler_1.UNIRedux.charm} ${activePet.petIcon} **${activePet.petName}** tried to cheat but failed.`;
                }
                break;
            case "bash": {
                const res = pet_fight_1.PetTurns.Bash(turnCTX);
                flavorText += res.flavor;
                break;
            }
            case "hexsmash": {
                const res = pet_fight_1.PetTurns.HexSmash(turnCTX);
                flavorText += res.flavor;
                break;
            }
            case "fluxstrike": {
                const res = pet_fight_1.PetTurns.FluxStrike(turnCTX);
                flavorText += res.flavor;
                break;
            }
            case "chaosbolt": {
                const res = pet_fight_1.PetTurns.ChaosBolt(turnCTX);
                flavorText += res.flavor;
                break;
            }
            case "laststand": {
                const res = pet_fight_1.PetTurns.LastStand(turnCTX);
                flavorText += res.flavor;
                break;
            }
            case "equilibrium":
                flavorText = `${cassidy_styler_1.UNIRedux.charm} ${activePet.petIcon} **${activePet.petName}** used ⚖️ **Equilibrium**!\n`;
                const eqFactor = Math.min(1 + petStats.totalDamageTaken / (activePet.maxHP * 2), 2);
                const hpDiff = targetPet.getPercentHP() - activePet.getPercentHP();
                if (hpDiff > 0) {
                    ...
                    const attackFactor = Math.min(attackStat / statThreshold, 1);
                    const defenseFactor = Math.min(defenseStat / statThreshold, 1);
                    const damage = Math.round(activePet.calculateAttack(targetPet.DF, (activePet.ATK + activePet.MAGIC) / 2) *
                        (hpDiff / 100) *
                        eqFactor *
                        attackFactor);
                    let heal = Math.round(((activePet.DF + activePet.MAGIC) / 4) *
                        (hpDiff / 100) *
                        eqFactor *
                        defenseFactor +
                    ...
                    const finalHeal = Math.min(heal, activePet.maxHP - activePet.HP, maxHeal);
                    ...
                    flavorText += `${cassidy_styler_1.UNIRedux.charm} Dealt **${finalDamage}** damage, healed **${finalHeal}** HP.\n${targetPet.getPlayerUI()}\n${activePet.getPlayerUI({ upperPop: `+${finalHeal} HP` })}`;
                    petStats.lastMove = "equilibrium";
                }
                else {
                    flavorText += `${cassidy_styler_1.UNIRedux.charm} No effect! Opponent's HP% not higher.`;
                }
                break;
            case "defend":
                flavorText = `${cassidy_styler_1.UNIRedux.charm} ${activePet.petIcon} **${activePet.petName}** used 🛡️ **Defend**!`;
                break;
            // case "lifeup":
            ...
            //   break;
            case "guardpulse":
                flavorText = `${cassidy_styler_1.UNIRedux.charm} ${activePet.petIcon} **${activePet.petName}** used 🛡️ **GuardPulse**!\n`;
                const guardFactor = Math.max(0.5, 1 - petStats.defenseBoosts * 0.2);
                const guardBoost = Math.round(activePet.DF *
                    (1 - activePet.HP / activePet.maxHP) *
                    1.5 *
                    guardFactor);
                activePet.defModifier += guardBoost;
                petStats.defenseBoosts += 1;
                flavorText += `${cassidy_styler_1.UNIRedux.charm} Defense boosted by **${guardBoost}**.\n${activePet.getPlayerUI({
                    showStats: true,
                })}`;
                break;
            case "vitalsurge":
                flavorText = `${cassidy_styler_1.UNIRedux.charm} ${activePet.petIcon} **${activePet.petName}** used 💖 **VitalSurge**!\n`;
                const healFactor = Math.min(1.5, 1 + (1 - petStats.healsPerformed * 0.2));
                const surgeHeal = Math.round(activePet.MAGIC *
                    (1 + activePet.HP / activePet.maxHP) *
                    0.5 *
                    healFactor);
                ...
                flavorText += `${cassidy_styler_1.UNIRedux.charm} Healed **${finalHeal}** HP.\n${activePet.getPlayerUI({
                    upperPop: `+${finalHeal} HP`,
                })}`;
                break;
            case "statsync":
                flavorText = `${cassidy_styler_1.UNIRedux.charm} ${activePet.petIcon} **${activePet.petName}** used 🔄 **StatSync**!\n`;
                const syncFactor = Math.max(0.5, 1 - petStats.attackBoosts * 0.2);
                const syncBoost = Math.round(Math.max(0, Math.min((activePet.DF + 1) *
                    (targetPet.DF / (activePet.DF || 1)) *
                    0.4 *
                    syncFactor, activePet.level * 2)));
                activePet.atkModifier += syncBoost;
                petStats.attackBoosts += 1;
                flavorText += `${cassidy_styler_1.UNIRedux.charm} ${syncBoost < 1
                    ? `ATK boost too weak.`
                    : `ATK boosted by **${syncBoost}**.`}\n${activePet.getPlayerUI({ showStats: true })}`;
                break;
            default:
                flavorText = `${cassidy_styler_1.UNIRedux.charm} ${activePet.petIcon} **${activePet.petName}** doesn't know **${turn}**.`;
        }
        if (gameState.activePlayer === 1)
            gameState.prevMove1 = turn;
        else
            gameState.prevMove2 = turn;
        if (targetPet.HP <= 0) {
            info.removeAtReply();
        ...
        const player1Data = await ctx.money.getItem(gameState.player1Author);
        const player2Data = await ctx.money.getItem(gameState.player2Author);
        gameState.flavorCache = `${gameState.activePlayer === 1 ? player1Data.name : player2Data.name}'s turn!`;
        let isCurrentAI = gameState.isAIMode && gameState.activePlayer === 2;
        let str = `${extraAIRes ? `${extraAIRes}\n` : ""}${flavorText}\n\n${targetPet.getPlayerUI({
            turn: isCurrentAI ? false : true,
            selectionOptions: petSchema,
            showStats: true,
        })}${isCurrentAI ? `\nAI is thinking...\n` : ""}`;
        if (!isCurrentAI) {
            str += `\n\n${gameState.activePlayer === 1 ? player1Data.name : player2Data.name}, select your move!\n\n⚠️ **Remaining turns before ending:  ${MAX_TURNS - (gameState.turnCount + 1)}**\n\n***Reply with one option (word only)***`;
        }
        if (!isCurrentAI) {
            const newInfo = await ctx.output.replyStyled(str, exports.style);
            newInfo.atReply(async (turnCtx) => await handlePlayerTurn(turnCtx, newInfo));
        }
        else {
    ...
    async function handleWin(ctx, winner, isMaxTurns = false, extraText = "") {
        if (!gameState || !gameState.player1Pet || !gameState.player2Pet)
            return;
        const winnerPet = winner === 1 ? gameState.player1Pet : gameState.player2Pet;
        const loserPet = winner === 1 ? gameState.player2Pet : gameState.player1Pet;
        const winnerId = winner === 1 ? gameState.player1Author : gameState.player2Author;
        const loserId = winner === 1 ? gameState.player2Author : gameState.player1Author;
        let winnerPts = Math.round((statMap.get(`${winnerId}_${winnerPet.OgpetData.key}`).totalDamageDealt /
            10) *
            1.5);
        ...
        const loserPts = Math.round(statMap.get(`${loserId}_${loserPet.OgpetData.key}`).totalDamageDealt / 10);
        ...
        const winnerName = winner === 2 && gameState.isAIMode ? "AI Opponent" : winnerData.name;
        const loserData = loserId
            ? await ctx.money.getItem(loserId)
            : { name: "AI Opponent", battlePoints: 0 };
        const loserName = winner === 1 && gameState.isAIMode ? "AI Opponent" : loserData.name;
        let wonDias = 0;
        if (!gameState.isAIMode || winner === 1) {
            const cll = new InventoryEnhanced_1.Collectibles(winnerData.collectibles);
            // astral momints
            ...
            const stre = calculatePetStrength(winnerPet) + calculatePetStrength(loserPet);
            const diasReward = Math.max(1, Math.min(100, Math.floor(stre / 2 / 500))) || -1;
            if (gameState.isAIMode) {
        ...
        await ctx.output.replyStyled((extraText ? `${extraText.trimEnd()}\n\n` : "") +
            (isMaxTurns
                ? `${cassidy_styler_1.UNIRedux.charm} Max turns reached!\n${winnerName} wins by having **higher remaining HP%**!\n${winnerPet.petIcon} **${winnerPet.petName}** had more health than ${loserPet.petIcon} **${loserPet.petName}**.\n${winnerName} earned ${(0, ArielUtils_1.formatCash)(winnerPts, "💷", true)}, and ${(0, ArielUtils_1.formatCash)(winnerCash, true)} **${wonDias
                    ? ` and **${wonDias}** 💎 stellar gems & gems & 🔮 intertwined fate`
                    : ""}**, ${loserName} earned ${(0, ArielUtils_1.formatCash)(loserPts, "💷", true)} and ${(0, ArielUtils_1.formatCash)(loserCash, true)}.`
                : `${cassidy_styler_1.UNIRedux.charm} ${winnerName} wins!\n${winnerPet.petIcon} **${winnerPet.petName}** defeated ${loserPet.petIcon} **${loserPet.petName}**!\n${winnerName} earned ${(0, ArielUtils_1.formatCash)(winnerPts, "💷", true)}, and ${(0, ArielUtils_1.formatCash)(winnerCash, true)} **${wonDias
                    ? ` and **${wonDias}** 💎 stellar gems & gems & 🔮 intertwined fate!`
                    : ""}**, ${loserName} earned ${(0, ArielUtils_1.formatCash)(loserPts, "💷", true)} and ${(0, ArielUtils_1.formatCash)(loserCash, true)}.`), exports.style);
        gameState = null;
    }
    async function handleDefeat(ctx, info, winner, isMaxTurns = false, extraText = "") {
        isDefeat = true;
        info.removeAtReply();
        await handleWin(ctx, winner, isMaxTurns, extraText);
    }
    async function handleNoTurns(ctx, info, extraText = "") {
        if (!gameState || !gameState.player1Pet || !gameState.player2Pet)
            return;
        info.removeAtReply();
        let winner = null;
        ...
        await handleWin(ctx, xis, true, extraText);
    }
}
