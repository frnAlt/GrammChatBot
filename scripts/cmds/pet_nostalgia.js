...
exports.style = exports.PET_LIMIT = exports.meta = void 0;
exports.calculateWorth = calculateWorth;
exports.autoUpdatePetData = autoUpdatePetData;
exports.entry = entry;
exports.findQuestEligiblePets = findQuestEligiblePets;
...
const SmartSpectra_1 = require("@cass-modules/SmartSpectra");
...
async function uncageReply({ input, output, Inventory, money, repObj, }) {
    ...
    const { name = "Unregistered", petsData: rawPetsData = [] } = await money.getItem(input.senderID);
    const petsData = new Inventory(rawPetsData);
    const all = await money.getAllCache();
    const allPetsData = new Inventory(Object.values(all)
        .flatMap((i) => i.petsData)
        .filter(Boolean), Infinity);
    if (input.senderID !== author) {
        return;
    }
    if (petsData.getAll().length >= exports.PET_LIMIT) {
        return output.replyStyled(`🐾 You can only have a maximum of ${exports.PET_LIMIT} pets!`, exports.style);
    }
    switch (type) {
        case "uncaging":
            await handleUncage();
    ...
    async function handleUncage() {
        ...
        if (!item) {
            return output.replyStyled(`🐾 Please go back and reply a correct number, thank you!`, exports.style);
        }
        const i = await output.replyStyled(`📄${item.icon} What would you like to name your **pet**? (no spaces pls)`, exports.style);
    ...
    async function handleRename() {
        ...
        const existingPet = allPetsData
            .getAll()
            .find((pet) => String(pet.name).toLowerCase() === String(newName).toLowerCase());
        if (existingPet) {
            return output.replyStyled(`🐾 Sorry, but that name was already **taken** for an existing ${existingPet.petType} ${existingPet.icon}, please go back and send a different one.`, exports.style);
        }
        petsData.addOne({
            ...item,
            type: "pet",
        ...
        input.delReply(detectID);
        return output.replyStyled(`🐾 Thank you **${name}** for successfully adopting ${item.icon} a new ${item.key} **${newName}**!\n🐾 Goodluck with your new pet!`, exports.style);
    }
}
async function renameReply({ input, output, Inventory, money, repObj, }) {
    const { author, petVentory, type, detectID } = repObj;
    const { name = "Unregistered", petsData: rawPetsData = [], inventory: rawInventory = [], } = await money.getItem(input.senderID);
    ...
    async function handleChoose() {
        ...
        const i = await output.replyStyled(`📄${item.icon} What would you like to rename your **pet**? (no spaces pls)`, exports.style);
        input.delReply(detectID);
        input.setReply(i.messageID, {
            author: input.senderID,
            // @ts-ignore
    ...
    async function handleRename() {
        ...
        if (!inventory.has("dogTag")) {
            return output.replyStyled(`A 🏷️ **Dog Tag** is required to perform this action.`, exports.style);
        }
        ...
        input.delReply(detectID);
        return output.replyStyled(`🐾 Thank you **${name}** for successfully renaming ${item.icon} your pet ${item.petType} to **${newName}**!\n🐾 Goodluck with your new pet's name!`, exports.style);
    }
}
exports.style = {
    title: {
...
const petFoodsII = [
    ...
    },
    {
        ...
        flavorText: "Icy treats from the highest peaks, made specifically for a Yeti.",
        price: 150,
        onPurchase({ moneySet }) {
            moneySet.inventory.push({
                name: "Snowflake Surprise",
    ...
    },
    {
        ...
        name: "Infernal Feast",
        key: "infernalFeast",
        flavorText: "Fiery meals fit for the underworld guardian, this is basically Phoenix Ember but there's 3 fires instead of 1. (for Cerberus)",
        price: 700,
        onPurchase({ moneySet }) {
            moneySet.inventory.push({
                name: "Infernal Feast",
    ...
    },
    {
        ...
        key: "celestialFeast",
        flavorText: "Heavenly meals for a majestic creature. (for griffin pet but not peter)",
        price: 900,
        onPurchase({ moneySet }) {
            moneySet.inventory.push({
                name: "Celestial Feast",
    ...
    },
    {
        ...
        flavorText: "Exquisite cuisine from the depths of the ocean, a perfect and only food for Krakens",
        price: 1100,
        onPurchase({ moneySet }) {
            moneySet.inventory.push({
                name: "Deep Sea Delicacy",
...
const petFoods = [
    ...
    },
    {
        ...
        name: "Cosmic Crunch",
        key: "cosmicCrunch",
        flavorText: "Tasty cosmic treats for your cosmic dragon.. or normal dragon.",
        price: 500,
        onPurchase({ moneySet }) {
            moneySet.inventory.push({
                name: "Cosmic Crunch+",
    ...
    },*/
    {
        ...
        name: "Phoenix Ember",
        key: "phoenixEmber",
        flavorText: "A radiant ember from the heart of a Phoenix's fire. Nourishes and invigorates your majestic pet, fueling its eternal flame and vibrant plumage.",
        price: 700,
        async onPurchase({ moneySet }) {
            moneySet.inventory.push({
                name: "Phoenix Ember+",
                key: "phoenixEmber",
                flavorText: "A mystical ember known for its transformative properties. When consumed, it imbues the Phoenix with renewed vitality, enhancing its fiery aura and majestic presence.",
                icon: "🔥",
                type: "phoenix_food",
                saturation: 70 * 60 * 1000,
                sellPrice: 2500,
...
const exFoods = [
    {
        name: "Beefy Bones 𝔼𝕏 ✦",
        key: "beefyBonesEX",
        flavorText: "Hearty bones infused with legendary marrow for your loyal hound.",
        icon: "🦴",
        type: "dog_food",
        price: 10,
        priceType: "cll:gems",
    ...
    },
    {
        ...
        flavorText: "Aromatic catnip treats that awaken your feline's hidden royalty.",
        icon: "🐭",
        type: "cat_food",
        price: 10,
        priceType: "cll:gems",
    ...
    },
    {
        ...
        flavorText: "A rare blend of enchanted insects—ideal for any discerning serpent.",
        icon: "🐜",
        type: "snake_food",
        price: 10,
        priceType: "cll:gems",
...
const mythicExFoods = [
    {
        ...
        flavorText: "Imbued with celestial essence, this hay nourishes unicorns beyond the veil of dreams.",
        icon: "🦄",
        type: "unicorn_food",
        price: 25,
        priceType: "cll:gems",
    ...
    },
    {
        name: "Yeti Yogurt 𝔼𝕏 ✦",
        key: "yetiYogurtEX",
        flavorText: "Frozen from glacial milk and enchanted with alpine spores. Only for the fluffiest.",
        icon: "🥶",
        type: "yeti_food",
        price: 18,
        priceType: "cll:gems",
...
function calculateWorth(pet, allCache) {
    const rate = (0, unitypes_1.calculateInflation)(allCache);
...
function calculateNextExp(petData) {
    const { lastExp = 0 } = petData;
    const currentLevel = lastExp < 10 ? 1 : Math.floor(Math.log2(lastExp / 10)) + 1;
...
async function confirmSell({ input, output, repObj, money, }) {
    const { petsData, newMoney, price, author, petToSell, code, petSells } = repObj;
    ...
    return output.reply(`😥${petToSell.icon} You successfully sold **${petToSell.name}** for ${(0, ArielUtils_1.formatCash)(price)}`);
}
async function entry(ctx) {
    const { input, output, money, Inventory, GearsManage, PetPlayer, UTShop, generateGift, prefix, args, commandName, } = ctx;
    const { name = "Chara", petsData: rawPetsData = [], inventory: rawInventory = [], gearsData: rawGearsData = [], carsData: rawCarsData = [], money: playerMoney = 0, petSells = 0, cassEXP: cxp, } = await money.getCache(input.senderID);
    ...
    const home = new spectral_home_1.SpectralCMDHome({
        ...
        },
        {
            ...
            async handler(_, __) {
                const inventory = new Inventory(rawInventory);
                const petVentory = new Inventory(rawInventory.filter((item) => item.type === "pet"));
                ...
                if (pets.length === 0) {
                    return output.reply([
                        ...
                        `1. 🐾 **Pet Bundle** ☆ (**Basic**)`,
                        `- **3,000**$ 💰`,
                        `${unisym_js_1.UNIRedux.charm} A bundle of pets for sale!`,
                        ``,
                ...
                pets.forEach((pet, index) => {
                    petList += `${index + 1}. ${pet.icon} **${pet.name}** (${pet.key})\n${unisym_js_1.UNIRedux.charm} ${pet.flavorText}\n`;
                });
                const i = await output.reply(`🐾 Here are your caged pets:\n\n${petList}\n\n🐾 Which pet would you like to uncage? Reply with a number!`);
                input.setReply(i.messageID, {
                    author: input.senderID,
                    // @ts-ignore
                    callback: uncageReply,
        ...
        },
        {
            ...
            async handler(_, __) {
                ...
                for (let pet of pets) {
                    ...
                    result += `${unisym_js_1.UNIRedux.charm} ${player.getPlayerUI(isPetHungry(pet) ? { upperPop: "Hungry" } : {})}
🗃️ ***Type***: ${pet.petType}
🧭 ***Level***: ${pet.level}
✨ ***Exp***: ${pet.lastExp ?? 0}/${calculateNextExp(pet)}
💵 **Worth**: ${(0, ArielUtils_1.formatCash)(calculateWorth(pet, money.cache))}
...
🔎 ***ID***: ${pet.key}\n\n`;
        ...
        },
        {
            ...
            async handler(_, __) {
                ...
                let cassEXP = new cassEXP_js_1.CassEXP(cxp);
                let pets = petsData.getAll();
                if (pets.length === 0) {
                    let result = "";
                    result += `🐾 You don't have any pets, try **uncaging** a pet if you have opened a bundle.`;
                    result += `\n\n🔎 **Suggested Step**:\nType **${prefix}${commandName}-uncage** without fonts to **uncage** pets from your inventory.`;
                    return mctx.output.replyStyled(result, exports.style);
                }
                ...
                if (!targetPetData || !targetPet) {
                    ...
                    if (result.item) {
                        ...
                        cassEXP = new cassEXP_js_1.CassEXP(mctx.user.cassEXP);
                        pets = petsData.getAll();
                    }
                }
                if (!targetPetData && !targetPet) {
                    return mctx.output.replyStyled(`🐾 No hungry pets.`, exports.style);
                }
                if (!targetPetData) {
                    return mctx.output.replyStyled(`❌ You don't have a pet named "${targetPet}"!`, exports.style);
                }
                const originalPet = autoUpdatePetData(JSON.parse(JSON.stringify(targetPetData)));
                if (!isPetHungry(targetPetData)) {
                    return mctx.output.replyStyled(`❌ **${targetPetData.name}** is not hungry!`, exports.style);
                }
                let targetFood = (foodKey === "--auto"
                    ? SmartSpectra_1.SmartPet.findFoods(targetPetData, inventory)[0]
                    : inventory.getOne(foodKey));
                if (!targetFood || !foodKey) {
                    const result = await mctx.output.selectItem({
                        items: SmartSpectra_1.SmartPet.findFoods(targetPetData, inventory),
                        style: exports.style,
                ...
                if (!targetFood && !foodKey) {
                    return output.reply(`🐾 You do not have **food** for this pet. You may **buy** foods from the **${prefix}${commandName}-shop** or other places.`);
                }
                if (!targetFood) {
                    return mctx.output.replyStyled(`❌ You don't have an inventory item that has key "${foodKey}"`, exports.style);
                }
                if (!SmartSpectra_1.SmartPet.isFeedable(targetPetData, targetFood)) {
                    return mctx.output.replyStyled(`❌ You can only feed a ${targetPetData.petType} with a food that has type: "${targetPetData.petType}_food", **${targetPetData.name}** will obviously not eat "${targetFood.type}" typed food.`, exports.style);
                }
                if ((targetPetData.lastFoodEaten === targetFood.key &&
                    (targetFood.picky ||
                        targetFood.key === "badApple" ||
                        targetFood.type === "food")) ||
                    (Number(targetFood.saturation) < 0 &&
                        Number(targetPetData.lastExp) < 0)) {
                    return mctx.output.replyStyled(`${unisym_js_1.UNIRedux.charm} ${targetPetData.icon} **${targetPetData.name}** no longer likes ${targetFood.icon} **${targetFood.name}**!\nPlease feed them **something else** before feeding it this **same food** again.\n\n(Did I bold too many words?)`, exports.style);
                }
                if (targetFood.type === "food") {
                    const sat1 = (Number(targetFood.heal) || 0) * 1.2 * 60 * 1000;
                    targetFood.saturation = Math.floor(sat1 * 0.25 + Math.floor(Math.random() * (sat1 * 0.75)) + 1);
                }
                ...
                if (targetFood.type === "food")
                    targetPetData.lastSaturation += targetFood.saturation;
                targetPetData.lastFeed = Math.min((targetPetData.lastFeed ?? Date.now()) +
                    targetFood.saturation * 360, Date.now());
                ...
                const userAddedExp = (0, unisym_js_1.clamp)(3, Math.floor(targetPetData.lastExp / 1000), 50);
...
✨ ***Exp***: ${updatedPet.lastExp ?? 0}/${calculateNextExp(updatedPet)} ${getDiff("lastExp")}
💵 **Worth**: ${(0, ArielUtils_1.formatCash)(calculateWorth(updatedPet, money.cache))} (+${(0, ArielUtils_1.formatCash)(Number(getDiff("worth")))})
🍽️ ***Hungry ${hungryAfter >= 0 ? `After` : `Since`}***: ${global.utils.convertTimeSentence(global.utils.formatTimeDiff(Math.abs(hungryAfter)))}${isPetHungry(updatedPet)
                    ? `\n⚠️ **WARN**: Please feed ${updatedPet.name} immediately.`
                    : ""}
🔎 ***ID***: ${updatedPet.key}`;
                return mctx.output.replyStyled(`✅ **${targetPetData.name}** has been fed with ${targetFood.icon === updatedPet.icon ? "" : `${targetFood.icon} `}**${targetFood.name}**!\n\nThis food effect will last for approximately ${Math.floor(targetFood.type === "food"
                    ? (Number(targetFood.saturation) / 60 / 1000) * 2
                    : Number(targetFood.saturation) / 60 / 1000)} minutes.\n\n${petText}\n\nThank you **${name}** for taking care of this pet!`, exports.style);
            },
        },
        {
            key: "gear",
            ...
            async handler(_, __) {
                ...
                petsData
                    .getAll()
                    .sort((a, b) => (Number(b.lastExp) || 0) - (Number(a.lastExp) || 0));
                if (args[0]) {
                    ...
                    let pet = allPetsData
                        .getAll()
                        .find((pet) => String(pet.name).toLowerCase().trim() ===
                        String(args[0]).toLowerCase().trim());
                    if (pet && !petsData.has(pet.key)) {
                        owner =
                            Object.values(all).find((i) => (i.petsData ?? []).some((i) => i?.key === pet?.key)) ?? owner;
                    }
                    if (!pet) {
                        return output.reply(`🐾 We don't have a pet named "${args[0]}"`);
                    }
                    const allGearsData = new GearsManage(Object.values(all)
                    ...
                    let result = `👤 **Owner**: ${owner.name}\n\n` +
                        `${petPlayer.getPlayerUI()}\n\n` +
                        `${unisym_js_1.UNIRedux.charm} ***Total Stats***\n\n`;
                    result +=
...
✨ ***Exp***: ${pet.lastExp ?? 0}/${calculateNextExp(pet)}
💵 **Worth**: ${(0, ArielUtils_1.formatCash)(calculateWorth(pet, money.cache))}\n\n` +
                            `${unisym_js_1.UNIRedux.charm} ***Gears***\n\n` +
...
💵 **Worth**: ${(0, ArielUtils_1.formatCash)(calculateWorth(pet, money.cache))}\n\n`;
        ...
        },
        {
            ...
            async handler(_, __) {
                ...
                if (args[0]) {
                    ...
                    let result = `${petPlayer.getPlayerUI()}\n\n` +
                        `${unisym_js_1.UNIRedux.charm} ***Elemental Info***\n\n` +
                        `${petPlayer.petIcon} **${petPlayer.petName}** (${petPlayer.petType}) belongs to **${elementals.elements
                    ...
                    for (const gap of gaps) {
                        result += `${gap.status === "stronger" ? "⚠️" : "⚡"} ${Math.round(Math.abs(gap.acc * 100))}% **${gap.status === "weaker" ? "stronger" : "weaker"}** vs ${gap.type}\n`;
                    }
                    result += `\n${unisym_js_1.UNIRedux.charm} ***Spells (Coming Soon)***\n\n`;
                    for (const spell of targetMap) {
                        const spellData = PetPlayer.spells[spell] ?? {};
                        result += `${spellData.icon ?? "⚡"} **${spellData.name ?? "Unknown"}** [ ${spellData.tp ?? 0}% ***TP*** ]\n${unisym_js_1.UNIRedux.charm} ${spellData.flavorText ?? "We don't know what this does..?"}\n\n`;
                    }
                    return output.reply(result);
                }
                let result = ``;
        ...
        },
        {
            ...
            async handler(_, __) {
                ...
                const petToSell = petsData
                    .getAll()
                    .find((pet) => pet?.name?.toLowerCase?.().trim() ===
                    nameToSell.toLowerCase().trim()) || petsData.getOne(nameToSell);
                if (!petToSell) {
                ...
                const gearData = gearsData.getGearData(updatedPet.key);
                if (gearData.hasGear()) {
                    return output.reply(`🐾 You cannot sell this pet, it has armors and weapons equipped.`);
                }
                if (updatedPet.level < 5) {
                    return output.reply(`🐾 Your pet is currently at level ${petToSell.level}, it must be at least level 5 to be sold.`);
                }
                ...
                const i = await output.reply(`🛡️ Please reply this 12-digit **code** to confirm the sale, make sure to type it **without fonts**.

[font=typewriter]${code}[:font=typewriter]

You are going to sell ${petToSell.icon} **${petToSell.name}** for $${(0, ArielUtils_1.formatCash)(price)}`);
                input.setReply(i.messageID, {
                    petsData,
                    newMoney,
                    code,
            ...
            async handler(_, __) {
                ...
                const sortedKeys = Object.keys(allData)
                    ...
                    const sortedB = dataB
                        ...
                        .sort((a, b) => calculateWorth(b, money.cache) +
                        (b.lastExp ?? 0) -
                    ...
                    const statA = new PetPlayer(highestA, petGearA).HP /
                        4 +
                        calculateWorth(highestA, money.cache) / 1000;
                    const statB = new PetPlayer(highestB, petGearB).HP /
                        4 +
                ...
                for (const userID of sortedKeys) {
                    const { gearsData = [], petsData = [], name = "Chara", } = allData[userID];
                    const pet = autoUpdatePetData(petsData.sort((a, b) => calculateWorth(b, money.cache) +
                        (Number(b.lastExp) || 0) -
                        (Number(calculateWorth(a, money.cache)) +
                            (Number(a.lastExp) ?? 0)))[0]);
                    const gearsManage = new GearsManage(gearsData);
                    const gearData = gearsManage.getGearData(pet.key);
                    const player = new PetPlayer(pet, gearData.toJSON());
                    result += `${num === 1 ? `👑` : num > 10 ? num : `0${num}`} ${num === 1
                        ? `[font=double_struck]${name
                            .toUpperCase()
                            .split("")
                            .join(" ")}[:font=double_struck]`
                        : `- ***${name}***`}\n${unisym_js_1.UNIRedux.charm} ${player.getPlayerUI(isPetHungry(pet) ? { upperPop: "Hungry" } : {})}
...
💵 **Worth**: ${(0, ArielUtils_1.formatCash)(calculateWorth(pet, money.cache))}\n\n`;
                ...
                result += `Type **${prefix}pet-top ${page + 1}** to view the next page.`;
                return output.reply(result);
            },
        },
        {
            ...
            async handler(_, __) {
                ...
                const petsData = new Inventory(rawPetsData);
                if (!inventory.has("dogTag")) {
                    return output.reply(`A 🏷️ **Dog Tag** is required to perform this action.`);
                }
                const pets = petsData.getAll();
                if (pets.length === 0) {
                    let result = "";
        ...
        },
        {
            ...
            async handler(_, __) {
                ...
                if (!carName || petNames.length === 0) {
                    return output.reply(`🐾 Please specify arguments with a **car name** and some **pet names** separated all by **spaces.**`);
                }
                ...
                for (const petName of petNames) {
                    ...
                    if (!pet) {
                        return output.reply(`❌ You don't have a pet named "${petName}"!`);
                    }
                    if (typeof pet.carAssigned === "string" &&
                ...
                if (currentPetCount + petsToAssign.length > 5) {
                    return output.reply(`🐾 You can only have a maximum of 5 pets in **${targetCar.name}**! (Current: ${currentPetCount})`);
                }
                ...
                return output.reply(`✅ Pets have been assigned to ${targetCar.icon || "🚗"} **${targetCar.name}**! Happy roadtrip!\n\n` +
        ...
        },
        {
            ...
            async handler(_, __) {
                ...
                if (!carName) {
                    ...
                    let result = `${unisym_js_1.UNIRedux.charm} Cars with Pets of **${name}**\n\n`;
                    for (const car of cars) {
                        const petIcons = (car.pets ?? [])
                            ...
                            .join(" ");
                        result +=
                            `${unisym_js_1.UNIRedux.charm} ${car.icon || "🚗"} **${car.name || "Unnamed"}**\n` +
                                // @ts-ignore
                                `💺 ***Passengers** ${car.pets.length}/5\n` +
                                `${unisym_js_1.UNIRedux.disc} ${petIcons || "None"}\n\n`;
                    }
                    result +=
                ...
                if (!Array.isArray(targetCar.pets) || targetCar.pets.length === 0) {
                    return output.reply(`🐾 No pets here, Type "${prefix}pet-addcar <car name> <...pet names>" to **assign now**.`);
                }
                const assignedPets = targetCar.pets
                    .map((petId) => petsData.getOne(petId))
                    .filter((pet) => pet && typeof pet === "object")
                    .sort((a, b) => (a.name || "").localeCompare(b.name || ""));
                let result = `${unisym_js_1.UNIRedux.charm} Pets in **${targetCar.icon || "🚗"} ${targetCar.name}** of **${name}**\n\n`;
...
💵 **Worth**: ${(0, ArielUtils_1.formatCash)(calculateWorth(updatedPet, money.cache))}
🍽️ ***Hungry ${hungryAfter >= 0 ? `After` : `Since`}***: ${global.utils.convertTimeSentence(global.utils.formatTimeDiff(Math.abs(hungryAfter)))}${isPetHungry(updatedPet)
                        ? `\n⚠️ **WARN**: Please feed ${updatedPet.name} immediately.`
                        : ""}
🔎 ***ID***: ${updatedPet.key}\n\n`;
        ...
        },
        {
            ...
            async handler(_, __) {
                ...
                let pets = petsData.getAll();
                if (pets.length === 0) {
                    return output.replyStyled(`🐾 You don't have any pets, try **uncaging** a pet if you have opened a bundle.\n\n🔎 **Suggested Step**:\nType **${prefix}${commandName}-uncage** without fonts to **uncage** pets from your inventory.`, exports.style);
                }
                let targetPetData;
                if (args[0]) {
                    targetPetData = petsData
                        ...
                        String(args[0]).toLowerCase().trim());
                    if (!targetPetData) {
                        return mctx.output.replyStyled(`❌ You don't have a pet named "${args[0]}"!`, exports.style);
                    }
                }
                else {
                    const result = await mctx.output.selectItem({
                    ...
                    if (result.item) {
                        ...
                        return mctx.output.replyStyled(`🐾 No pets are eligible for a quest right now.`, exports.style);
                    }
                }
                const updatedPet = autoUpdatePetData(targetPetData);
                const currentTime = Date.now();
                ...
                if (updatedPet.questCount >= QUEST_CONFIG.DAILY_QUEST_LIMIT) {
                    return mctx.output.replyStyled(`🐾 **${updatedPet.name}** has reached the daily quest limit (${QUEST_CONFIG.DAILY_QUEST_LIMIT}). Try again tomorrow!`, exports.style);
                }
                const timeSinceLastQuest = currentTime - updatedPet.lastQuest;
                const timeFactor = Math.min(timeSinceLastQuest / QUEST_CONFIG.COOLDOWN, 1);
                if (timeSinceLastQuest > QUEST_CONFIG.STREAK_RESET_THRESHOLD) {
                    updatedPet.questStreak = 0;
                }
                updatedPet.questStreak = Math.min(updatedPet.questStreak + 1, QUEST_CONFIG.MAX_STREAK);
                const worth = calculateWorth(updatedPet, money.cache);
                const baseReward = Math.floor(worth * QUEST_CONFIG.REWARD_MULTIPLIER * timeFactor);
                const bonus = QUEST_CONFIG.BASE_BONUS +
                ...
                petsData.deleteOne(updatedPet.key);
                petsData.addOne(updatedPet);
                const newMoney = (await money.queryItem(mctx.input.senderID, "money")).money +
                    reward;
                await money.setItem(mctx.input.senderID, {
                    money: newMoney,
                    petsData: Array.from(petsData),
                ...
                const questResults = [
                    `${updatedPet.icon} **${updatedPet.name}** found a treasure chest worth ${(0, ArielUtils_1.formatCash)(reward)}!`,
                    `${updatedPet.icon} **${updatedPet.name}** won a pet show, earning ${(0, ArielUtils_1.formatCash)(reward)}!`,
                    `${updatedPet.icon} **${updatedPet.name}** completed a daring task for ${(0, ArielUtils_1.formatCash)(reward)}!`,
                ];
                const resultText = questResults[Math.floor(Math.random() * questResults.length)];
                const timeUntilReady = QUEST_CONFIG.COOLDOWN - timeSinceLastQuest;
                const nextQuestText = timeUntilReady <= 0
                    ? `Available now`
                    : `In ${global.utils.convertTimeSentence(global.utils.formatTimeDiff(timeUntilReady))}`;
                return mctx.output.replyStyled(`✅ ${resultText}\n\n` +
                    `💵 **Reward**: ${(0, ArielUtils_1.formatCash)(reward)} (Base: ${(0, ArielUtils_1.formatCash)(baseReward)} + Streak Bonus: ${(0, ArielUtils_1.formatCash)(bonus)})\n` +
                    `⏳ ***Readiness***: ${(timeFactor * 100).toFixed(0)}% (Full reward ${nextQuestText})\n` +
                    `🔄 ***Streak***: ${updatedPet.questStreak} (Next bonus: ${(0, ArielUtils_1.formatCash)(bonus + QUEST_CONFIG.STREAK_INCREMENT)})\n` +
                    `📅 **Daily Quests Left**: ${QUEST_CONFIG.DAILY_QUEST_LIMIT - updatedPet.questCount}/${QUEST_CONFIG.DAILY_QUEST_LIMIT}\n` +
...
function findQuestEligiblePets(petsData) {
    const dayStart = new Date().setUTCHours(0, 0, 0, 0);
    return petsData
        .getAll()
        .filter((pet) => {
        const updatedPet = autoUpdatePetData(pet);
        const isUnderDailyLimit = updatedPet.lastQuestDay !== dayStart
            ? true
