...
async function entry({ cancelCooldown, output, args, prefix, commandName, uid, usersDB, userName, input, }) {
    if (!args[0]) {
        cancelCooldown();
        return output.reply(`❌ Please enter a caption for a fake post.
**Example**: ${prefix}${commandName} Ang cute mo bwiset.`);
    ...
    while (true) {
        ...
        try {
            ...
            const margin2 = 10;
            const ym = 3;
            const header = CassieahExtras_1.CanvCass.createRect({
                height: pfps + margin * 2,
                width: canv.width - margin * 2,
                left: canv.left + margin,
                top: canv.top + margin * 2,
            });
            const pfpBox = CassieahExtras_1.CanvCass.createRect({
                width: pfps,
                height: pfps,
                left: header.left,
                top: header.top,
            });
            const pfpPath = CassieahExtras_1.CanvCass.createCirclePath([pfpBox.centerX, pfpBox.centerY], pfpBox.width / 2);
            await utils.delay(500);
            const pfp = await CassieahExtras_1.CanvCass.loadImage(pfpURL);
            ...
            canv.drawLine([canv.left, header.top - margin], [canv.right, header.top - margin], {
                stroke: lw2,
                strokeWidth: 2,
            });
            canv.drawLine([canv.left, header.top - margin - 7], [canv.right, header.top - margin - 7], {
                stroke: lw2,
                strokeWidth: 2,
            });
            canv.drawText(name, {
            ...
            canv.drawText(`${(0, node_crypto_1.randomInt)(1, 12)}h  •  Public`, {
                vAlign: "bottom",
                y: pfpBox.centerY + ym,
                x: pfpBox.right + margin2,
                align: "left",
            ...
            const reactsize = 25;
            const wallImg = CassieahExtras_1.CanvCass.createRect({
                height: canv.width / (16 / 9),
                width: canv.width - margin * 2,
                centerX: canv.centerX,
                top: header.bottom - margin,
            });
            const gradient = canv.tiltedGradient(wallImg.width, wallImg.height, -Math.PI / 4, [
                [0, "rgba(255, 0, 71, 1)"],
                [1, "rgba(44, 52, 199, 1)"],
            ]);
            canv.drawBox({
            ...
            canv.withClip(CassieahExtras_1.CanvCass.rectToPath(wallImg), () => {
            ...
            const footer = CassieahExtras_1.CanvCass.createRect({
                height: buttonss + margin * 2,
                width: canv.width,
                centerX: canv.centerX,
                top: wallImg.bottom + reactsize + 4,
            });
            canv.drawText(`❤️👍 ${(0, node_crypto_1.randomInt)(3, 576)}K`, {
                size: reactsize,
                fontType: "cnormal",
                align: "left",
            ...
            for (let i = 0; i < amount; i++) {
                ...
                const rect = CassieahExtras_1.CanvCass.createRect({
