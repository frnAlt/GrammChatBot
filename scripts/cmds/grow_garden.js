...
exports.style = exports.ITEMS_PER_PAGE = exports.PET_EQUIP_LIMIT = exports.PET_LIMIT = exports.PLOT_EXPANSION_LIMIT = exports.PLOT_LIMIT = exports.briefcaseUsage = exports.treasuresTable = exports.meta = void 0;
exports.toBarnItem = toBarnItem;
exports.calculateCropValue = calculateCropValue;
...
const unisym_js_1 = require("@cass-modules/unisym.js");
...
const GardenShop_1 = require("@cass-modules/GardenShop");
// ---- Garden Configuration I wanna move somewhere else ----
const GardenConfig_1 = require("@cass-modules/GardenConfig");
const GardenEventConfig_1 = require("@cass-modules/GardenEventConfig");
...
const unitypes_1 = require("@cass-modules/unitypes");
// ---- Do not ask wth this was ----
const Datum_1 = require("@cass-modules/Datum");
// ---- Hacky stuff ----
const GardenBalancer_1 = require("@cass-modules/GardenBalancer");
exports.meta = {
    name: "garden",
...
exports.treasuresTable = [
    ...GardenEventConfig_1.EVENT_CONFIG.ALL_EVENTS.filter((i) => i.key !== "bizzyBees")
        .map((i) => i.shopItems)
        .flat()
        .map((shopItem) => {
    ...
    ...GardenShop_1.gardenShop.honeyShop
...
// ---- For bc use ----
exports.briefcaseUsage = {
    gardenSeed(arg, ctx, _bctx) {
        const item = arg.item;
        return `${unisym_js_1.UNISpectra.arrowFromT} ${item.icon} **${item.name}** is a garden seed! You can **plant** this item.\n\n${unisym_js_1.UNIRedux.charm} Base Value: ${(0, ArielUtils_1.formatCash)(item.cropData?.baseValue, true)}\n\nType **${ctx.prefix}${exports.meta.name} plant ${item.key} ${arg.customInventory.getAmount(item.key)}** without fonts to plant it.`;
    },
    gardenPet(arg, ctx, _bctx) {
        const item = arg.item;
        return `${unisym_js_1.UNISpectra.arrowFromT} ${item.icon} **${item.name}** is a garden pet! You can **uncage** this pet.\n\nType **${ctx.prefix}${exports.meta.name} uncage ${item.key}** without fonts to uncage it.`;
    },
    gardenTool(arg, _ctx, _bctx) {
        const item = arg.item;
        return `${unisym_js_1.UNISpectra.arrowFromT} ${item.icon} **${item.name}** is a garden gear! Just **keep** it in your inventory/briefcase and it will have an **effect**.`;
    },
};
exports.PLOT_LIMIT = 36;
exports.PLOT_EXPANSION_LIMIT = 56 * 2;
exports.PET_LIMIT = 60;
exports.PET_EQUIP_LIMIT = 8;
exports.ITEMS_PER_PAGE = 6;
function toBarnItem(plot, inflationRate) {
    const { key, icon, name, flavorText, mutation, kiloGrams, seedKey, isFavorite, price, } = plot;
...
function calculateCropValue(crop, inflationRate) {
    const mutations = (crop.mutation || [])
        .map((mutationName) => GardenConfig_1.CROP_CONFIG.MUTATIONS.find((m) => m.name === mutationName))
        .filter(Boolean);
    const totalMutationBonus = mutations.reduce((acc, curr) => acc * (curr.valueMultiplier || 1), 1);
    const yields = getPossibleYields(crop);
    const combinedMultiplier = totalMutationBonus;
    const final = Math.floor(crop.baseValue * combinedMultiplier * (crop.maxKiloGrams + 1));
...
function cropTimeLeft(plot, allowNegative = false) {
...
function calculateCropKG(crop) {
    const percent = getCropFullnessPercent(crop) + getOvergrownBonusPercent(crop) / 2;
    const rawKG = crop.maxKiloGrams * percent;
    return Number(rawKG.toFixed(2));
}
function getPossibleYields(crop) {
    const timeLeft = cropTimeLeft(crop, true);
    if (cropTimeLeft(crop, true) > 0)
        return 0;
    const growthTimePerYield = crop.growthTime;
    if (growthTimePerYield <= 0)
        return 0;
...
async function autoUpdateCropData(crop, tools, pets) {
    if (!crop)
        return null;
    crop.lastUpdated ??= Date.now();
    crop.yields ??= 1;
    crop.mutationAttempts ??= 0;
    crop.maxKiloGrams ??= (0, unitypes_1.randomBiased)(GardenConfig_1.CROP_CONFIG.MIN_KG, GardenConfig_1.CROP_CONFIG.MAX_KG, GardenConfig_1.CROP_CONFIG.KILO_BIAS);
    crop.maxKiloGrams = Math.max(Math.min(GardenConfig_1.CROP_CONFIG.MAX_KG, crop.maxKiloGrams), GardenConfig_1.CROP_CONFIG.MIN_KG);
    ...
    const allowM = crop.lastMutation
        ? now - crop.lastMutation >= GardenConfig_1.CROP_CONFIG.MUTATION_INTERVAL
        : true;
    if (isOver &&
        allowM &&
        crop.mutationAttempts <= GardenConfig_1.CROP_CONFIG.MAX_MUTATION_ATT) {
        const maxCycles = Math.floor(GardenConfig_1.CROP_CONFIG.MAX_AFK / GardenConfig_1.CROP_CONFIG.MUTATION_INTERVAL);
        const timeSinceMutation = crop.lastMutation
            ? now - crop.lastMutation
            : GardenConfig_1.CROP_CONFIG.MUTATION_INTERVAL;
        const repeats = Math.min(Math.floor(timeSinceMutation / GardenConfig_1.CROP_CONFIG.MUTATION_INTERVAL), maxCycles);
        const skipStamp = (await Cassidy.databases.globalDB.getCache("skipStamp"))?.skipStamp || 0;
        const cycle = GardenEventConfig_1.EVENT_CONFIG.WEATHER_CYCLE_NEW;
        const adjustedNow = now + skipStamp;
        const currentCycle = Math.floor(adjustedNow / cycle);
        for (let i = 0; i < repeats; i++) {
            const simulatedTime = crop.lastMutation + i * GardenConfig_1.CROP_CONFIG.MUTATION_INTERVAL;
    ...
    crop.growthTime = Math.floor(Math.max(crop.originalGrowthTime / Math.max(growthBoost, 1), crop.originalGrowthTime * 0.25));
    if ((crop.mutation ?? []).length > 0 && crop.harvestsLeft > 1) {
        crop.growthTime = Math.floor(crop.growthTime * GardenConfig_1.CROP_CONFIG.OVERGROWTH_PENALTY);
    }
    for (const mutation of GardenConfig_1.CROP_CONFIG.MUTATIONS) {
        if (Array.isArray(mutation.both)) {
            if (mutation.both.every((i) => crop.mutation.includes(i)) &&
...
async function getCurrentWeather() {
    ...
    const weekNumber = Math.floor(adjustedNow / GardenEventConfig_1.EVENT_CONFIG.WEATHER_CYCLE_NEW) %
        GardenEventConfig_1.EVENT_CONFIG.WEATHERS.length;
    const event = GardenEventConfig_1.EVENT_CONFIG.WEATHERS[weekNumber];
    return event;
}
async function getCurrentEvent() {
    const event = GardenEventConfig_1.EVENT_CONFIG.CURRENT_EVENT;
    return event;
}
async function getNextWeather(skips = 1) {
    const { globalDB } = Cassidy.databases;
    const { skipStamp = 0 } = await globalDB.getCache("skipStamp");
    const adjustedNow = Date.now() + skipStamp;
    const cycle = GardenEventConfig_1.EVENT_CONFIG.WEATHER_CYCLE_NEW;
    const totalCyclesPassed = Math.floor(adjustedNow / cycle) + skips;
    const len = GardenEventConfig_1.EVENT_CONFIG.WEATHERS.length;
    const weekNumber = ((totalCyclesPassed % len) + len) % len;
    const event = GardenEventConfig_1.EVENT_CONFIG.WEATHERS[weekNumber];
    return event;
}
function forgivingRandom(bias) {
    return Math.random() ** (bias || 1.2);
}
async function skipWeather(skipped, targetTimeLeft = GardenEventConfig_1.EVENT_CONFIG.WEATHER_CYCLE_NEW) {
    const { globalDB } = Cassidy.databases;
    const cycle = GardenEventConfig_1.EVENT_CONFIG.WEATHER_CYCLE_NEW;
    const now = Date.now();
    const { skipStamp: currentSkipStamp = 0 } = await globalDB.getCache("skipStamp");
...
async function applyMutation(crop, tools, pets, accum = false, weather) {
    if (!accum) {
        crop.mutation = [];
    }
    const mutationBoosts = new Map();
    GardenConfig_1.CROP_CONFIG.MUTATIONS.forEach((mutation) => {
    ...
    const mutationTypeSet = new Set(mutationEffects.map((e) => e.mutationType).filter(Boolean));
    const mutations = [
        ...Array.from({ length: GardenConfig_1.CROP_CONFIG.MBIAS }, () => GardenConfig_1.CROP_CONFIG.MUTATIONS.filter((m) => mutationTypeSet.has(m.name))).flat(),
        ...GardenConfig_1.CROP_CONFIG.MUTATIONS.filter((m) => !mutationTypeSet.has(m.name)),
    ];
    for (const mutation of mutations) {
        if (!mutation)
            continue;
        const matchingEffect = mutationEffects.find((e) => e.mutationType === mutation.name);
        let mchance = matchingEffect?.mutationChance ?? mutation.chance;
        if (Array.isArray(mutation.pet)) {
            if (mutation.pet.some((i) => pets.has(i)) &&
                !crop.mutation.includes(mutation.name)) {
        ...
        const boost = mchance === 0
            ? 0
            : Math.min(0.99, mutationBoosts.get(mutation.name) ?? 0);
        let ratio = crop.kiloGrams / GardenConfig_1.CROP_CONFIG.MAX_KG;
...
function getMutation(name) {
    return GardenConfig_1.CROP_CONFIG.MUTATIONS.find((m) => m.name === name);
}
function formatMutationStr(plot) {
    if (!plot) {
        return ` ❔ **Nothing**`;
    }
    return `${(plot.mutation ?? []).length > 0
        ? `[ ${plot.mutation
            .map((i) => `${getMutation(i)?.icon ?? "❓"} ${cassidy_styler_1.FontSystem.fonts.double_struck(i)}`)
            .join(" + ")} ] `
        : ""}${plot.icon} **${plot.name}** (${plot.kiloGrams}kg)${plot.isFavorite ? " 💖" : ""}`;
}
...
// }
async function checkAchievements(user, money, output, input) {
    ...
    for (const achievement of GardenConfig_1.CROP_CONFIG.ACHIEVEMENTS) {
        if (!user.gardenStats?.achievements?.includes(achievement.key)) {
            ...
            else if (achievement.key === "expand_1" &&
    ...
    if (achievementsUnlocked.length > 0) {
        ...
        if (!input.isWeb) {
            await output.replyStyled(`🏆 **Achievements Unlocked**:\n${achievementsUnlocked.join("\n")}\n\n💰 New Balance: ${(0, ArielUtils_1.formatCash)(newMoney)}`, exports.style);
        }
    }
}
let officialUpdatedAt = null;
...
async function refreshShopStock(force = false) {
    ...
    let stocks;
    if (USE_TRUE_STOCK) {
        stocks = await (0, GardenConfig_1.fetchSeedStock)();
        if (typeof stocks.updatedAt === "number") {
            ...
            const timeLeft = (Math.abs(timePassed) % GardenShop_1.gardenShop.stockRefreshInterval) + 1000;
            GardenShop_1.gardenShop.lastRestock = currentTime - Math.abs(timeLeft);
            if (getTimeUntilRestock() <= 0) {
                GardenShop_1.gardenShop.lastRestock = currentTime;
            }
        }
        else {
            GardenShop_1.gardenShop.lastRestock = currentTime;
    ...
    GardenShop_1.gardenShop.eventItems = [];
    if (event.shopItems && event.shopItems.length > 0) {
        event.shopItems.forEach((shopItem) => {
            if (!GardenShop_1.gardenShop.eventItems.some((item) => item.key === shopItem.key)) {
                GardenShop_1.gardenShop.eventItems.push({ ...shopItem, isEventItem: true });
            }
        });
    }
    const getStock = (item) => {
        return typeof item.minStock === "number" &&
            ...
            ? Math.ceil((0, unitypes_1.randomBiased)(item.minStock, item.maxStock, GardenConfig_1.CROP_CONFIG.STOCK_MIN_BIAS))
            : Infinity;
    };
    for (const item of GardenShop_1.gardenShop.itemData) {
        let fetched = stocks && USE_TRUE_STOCK && Array.isArray(stocks.seeds)
            ? stocks.seeds.find((i) => item.name.includes(i) ||
    ...
    GardenShop_1.gardenShop.eventItems.forEach((item) => {
        item.inStock = forgivingRandom() < item.stockChance;
        item.stockLimit = getStock(item);
    });
    GardenShop_1.gardenShop.gnpShop.forEach((item) => {
        item.inStock = forgivingRandom() < item.stockChance;
        item.stockLimit = getStock(item);
    });
    return true;
...
function getTimeUntilRestock() {
    const currentTime = Date.now();
    const timePassed = currentTime - GardenShop_1.gardenShop.lastRestock;
    const timeLeft = GardenShop_1.gardenShop.stockRefreshInterval - timePassed;
    return Math.max(0, timeLeft);
}
function formatShopItems(items = GardenShop_1.gardenShop, currentEvent, isEvent = false) {
    const timeText = `🕒 **Next Restock**:\n${(0, ArielUtils_1.formatTimeSentence)(getTimeUntilRestock())}`;
    return {
        ...items,
        ...
        itemData: items.itemData.map((item) => {
            ...
            const moneySet = { inventory: [] };
            item.onPurchase({ moneySet });
            const purchased = moneySet.inventory[0];
            if (purchased) {
                if (purchased.type === "gardenSeed") {
                    flavor = `${`***${item.rarity}*** - ${Math.round((item.stockChance ?? 1) * 100)}%`.toUpperCase()}\n🪙 ${(0, ArielUtils_1.abbreviateNumber)(purchased.cropData.baseValue || 0)} | 🧺 ${(0, ArielUtils_1.abbreviateNumber)(purchased.cropData.harvests || 0)} | ⏳ ${(0, ArielUtils_1.formatTimeSentence)(purchased.cropData.growthTime || 0) || "Instant"}`;
                }
            }
        ...
        ...(currentEvent?.isNoEvent !== true && isEvent && currentEvent
            ? {
                welcomeTexts: [
                    `Welcome to the ${currentEvent?.icon ?? "🌱"} **${currentEvent.shopName2 ?? "Event Shop."}!**`,
                ],
...
function correctItems(rawInv) {
    const allItems = GardenEventConfig_1.EVENT_CONFIG.allItems;
...
function correctPlot(plot) {
    const allItems = GardenEventConfig_1.EVENT_CONFIG.allItems;
    const found = allItems.find((i) => i?.key === plot?.seedKey);
    if (found) {
        const temp = [];
        found.onPurchase({ moneySet: { inventory: temp } });
        const foundSeed = temp[0];
        if (foundSeed && foundSeed.key === plot.seedKey && foundSeed.cropData) {
            if (plot.baseValue !== foundSeed.cropData.baseValue) {
                ...
                plot.price =
                    found.priceType !== "money" && found.priceType
                        ? 0
                        : Math.min(foundSeed.cropData.baseValue || 0, (found.price ?? foundSeed.cropData.baseValue) /
            ...
            if (plot.maxKiloGrams <
                (foundSeed.cropData.baseKG ?? 0) + GardenConfig_1.CROP_CONFIG.MIN_KG) {
                plot.maxKiloGrams = (0, unitypes_1.randomBiased)(GardenConfig_1.CROP_CONFIG.MIN_KG + (foundSeed.cropData.baseKG ?? 0), GardenConfig_1.CROP_CONFIG.MAX_KG, GardenConfig_1.CROP_CONFIG.KILO_BIAS);
            }
        }
    }
    return plot;
...
async function entry(ctx) {
    ...
    const { input, output, money, Inventory, UTShop, prefix, commandName, command, fonts, getInflationRate, } = ctx;
    await money.ensureUserInfo(input.senderID);
    const inflationRate = await getInflationRate();
    await (async () => {
        const { money: balance, gardenPlots: rawPlots = [] } = await money.getCache(input.senderID);
    ...
    let { name = "", gardenPlots: rawPlots = [], gardenBarns: rawBarns = [], gardenPets: rawPets = [], gardenHeld = "", inventory: rawInventory = [], money: userMoney = 0, gardenStats: _gs = {
        plotsHarvested: 0,
        mutationsFound: 0,
        expansions: 0,
        achievements: [],
    }, plotLimit = exports.PLOT_LIMIT, lastSideExpansion = 0, lastRearExpansion1 = 0, lastRearExpansion2 = 0, 
    // allowGifting = true,
    gardenEarns = 0, collectibles: rawCLL, } = await money.getCache(input.senderID);
    const gardenStats = _gs;
    let isHypen = !!input.propertyArray[0];
    const collectibles = new InventoryEnhanced_1.Collectibles(rawCLL);
    correctItems(rawInventory);
    const exiTool = new Inventory(rawInventory.filter((item) => item.type === "gardenTool"));
    const exiPets = new Inventory(rawPets, exports.PET_LIMIT);
    rawPlots.forEach((i) => autoUpdateCropData(i, exiTool, exiPets));
    ...
    const xBarn = new Inventory(rawBarns, GardenConfig_1.CROP_CONFIG.BARN_LIMIT);
    const currentHeld = xBarn.getOneByID(gardenHeld);
    const style = {
        ...command.style,
        title: {
            content: `${currEvent.icon} ${unisym_js_1.UNISpectra.charm} **G🍓rden**`,
            text_font: "fancy",
            line_bottom: "default",
        },
        footer: {
            content: hasWeather
                ? `${currWeather.icon} Weather: **${currWeather.name}**${currWeather.effects?.length
                    ? " - " +
                        currWeather.effects
                            .map((e) => `${Math.round((e.mutationChance ?? 0) * 100)}% ${e.mutationType} mutations`)
                            .join(", ")
                    : ""}`
    ...
    if (!name || name === "Unregistered") {
        return output.reply(`🌱 Please register first!\nUse: **${prefix}register** without fonts.`);
    }
    ...
    const relapsedConfig = [
        {
            ...
            async handler(_, {}) {
                ...
                const choices = [
                    {
                        ...
                        async callback(rep) {
                            ...
                            const cll = new InventoryEnhanced_1.Collectibles(collectibles);
                            return rep.output.reply(`${unisym_js_1.UNISpectra.charm} ⚡ Here's your energy!\n\n**${cll.getAmount("relapseEnergy")}**/${maxEnergy}`);
                        },
                    },
                    {
                        txt: `I want to convert 🫳 this to an energy.`,
                        async callback(rep) {
                            let { gardenBarns = [], gardenHeld = "", collectibles, } = await rep.usersDB.getCache(rep.uid);
                            const cll = new InventoryEnhanced_1.Collectibles(collectibles);
                            const barns = new Inventory(gardenBarns, GardenConfig_1.CROP_CONFIG.BARN_LIMIT);
                            const item = barns.getOneByID(gardenHeld);
                            if (!item || !item.mutation.includes("Relapsed")) {
                                return rep.output.reply(`${unisym_js_1.UNISpectra.charm} ⚡ No held **RELAPSED** plant!\n\n💡 Hint: try opening your garden barn and hold an item!`);
                            }
                            if (item.isFavorite) {
                                return rep.output.reply(`${unisym_js_1.UNISpectra.charm} ⚡ Cannot give favorited plant!\n\n💡 Hint: try opening your garden barn and unfavorite an item!`);
                            }
                            const am = cll.getAmount("relapseEnergy");
                            if (am >= maxEnergy) {
                                return rep.output.reply(`${unisym_js_1.UNISpectra.charm} ⚡ Energy is already **${am}**/${maxEnergy}! Try playing some minigames, I guess.`);
                            }
                            const energyAdded = Math.min(Math.round(item.kiloGrams * kgToEnergy), maxEnergy - am);
                            ...
                            return rep.output.reply(`${unisym_js_1.UNISpectra.charm} ⚡ Added **${energyAdded}** energy!\n\n**${cll.getAmount("relapseEnergy")}**/${maxEnergy}`);
                        },
                    },
                    {
                        txt: `How do I use my energy?`,
                        async callback(rep) {
                            return (0, GardenShop_1.GardenChoice)({
                                style,
                                choices,
                ...
                return (0, GardenShop_1.GardenChoice)({ title: str, choices, style })(ctx);
            },
        },
        {
            key: "minigame",
                    ...
                    },
                    {
                        ...
                        async callback(rep) {
                            ...
                            if (cll.getAmount("relapseEnergy") < energyCost) {
                                return rep.output.reply(`${unisym_js_1.UNISpectra.charm} ${targ.icon} Not enough energy. You had ⚡ **${cll.getAmount("relapseEnergy")}** but we need ⚡ **${energyCost}**`);
                            }
                            cll.raise("relapseEnergy", -energyCost);
                            await rep.usersDB.setItem(rep.uid, {
                                collectibles: [...cll],
                            });
                            return (0, GardenEventConfig_1.PlayRelapseMinigame)(rep, style);
                        },
                    },
                ];
                return (0, GardenShop_1.GardenChoice)({ title, choices, style })(ctx);
            },
        },
    ];
    const beeCombpressor = {
        key: "combpressor",
        ...
        async handler(_, {}) {
            ...
            const choices = [
                {
                    ...
                    async callback(rep) {
                        let { honeyStamp, honeyKG: honeyKG_ = 0 } = await rep.usersDB.getCache(rep.uid);
                        let honeyKG = honeyKG_;
                        if (typeof honeyStamp === "number") {
                            return rep.output.reply(`${unisym_js_1.UNISpectra.charm} 🍯🏠 The combpressor is currently running! Try to collect it.`);
                        }
                        return rep.output.reply(`${unisym_js_1.UNISpectra.charm} 🍯🏠 **${Number(honeyKG.toFixed(2))}**/${neededKg} KG`);
                    },
                },
                {
                    txt: `I want to put 🫴 this to the honey combpressor.`,
                    async callback(rep) {
                        let { gardenBarns = [], honeyStamp, gardenHeld = "", honeyKG: honeyKG_ = 0, name = "User", } = await rep.usersDB.getCache(rep.uid);
                        let honeyKG = honeyKG_;
                        if (typeof honeyStamp === "number") {
                            return rep.output.reply(`${unisym_js_1.UNISpectra.charm} 🍯🏠 The combpressor is currently running! Try to collect it.`);
                        }
                        ...
                        if (!isAlreadyFull) {
                            if (!item || !item.mutation.includes(neededMutation)) {
                                return rep.output.reply(`${unisym_js_1.UNISpectra.charm} 🍯🏠 No held **POLLINATED** plant!\n\n💡 Hint: try opening your garden barn and hold an item!`);
                            }
                            if (item.isFavorite) {
                                return rep.output.reply(`${unisym_js_1.UNISpectra.charm} 🍯🏠 Cannot give favorited plant!\n\n💡 Hint: try opening your garden barn and unfavorite an item!`);
                            }
                        ...
                        if (honeyKG >= neededKg) {
                            ...
                            setTimeout(() => {
                                if (!rep.input.isWeb) {
                                    ...
                                    delete st.footer;
                                    const y = (0, GardenShop_1.GardenChoice)({
                            ...
                            return rep.output.reply(`${unisym_js_1.UNISpectra.charm} 🍯🏠 ${lineCompleteKG.randomValue()}\n\n${!isAlreadyFull
                                ? `🫴 ${formatMutationStr(item)}\n✅ Added **${kg} kilograms** to the combpressor.\n\n`
                                : "✅ Already full, no need for a plant!\n\n"}🍯🏠 **${Number(honeyKG.toFixed(2))}**/${neededKg} KG\n\nThe combpressor will start making honey, collect it after **${timeNeed / 1000} seconds.**`);
                        }
                        else {
                            await rep.usersDB.setItem(rep.uid, {
                                gardenBarns: barns.raw(),
                            ...
                            return rep.output.reply(`${unisym_js_1.UNISpectra.charm} 🍯🏠 ${linesNeedKG.randomValue()}\n\n${!isAlreadyFull
                                ? `🫴 ${formatMutationStr(item)}\n✅ Added **${kg} KG** to the combpressor.\n\n`
                                : ""}🍯🏠 **${Number(honeyKG.toFixed(2))}**/${neededKg} KG\n\n💡 ${honeyKG - neededKg} KG left!`);
                        }
                    },
                },
                {
                    ...
                    async callback(rep) {
                        const { collectibles: putCll = [], honeyStamp } = await rep.usersDB.getCache(rep.uid);
                        if (typeof honeyStamp !== "number") {
                            return rep.output.reply(`${unisym_js_1.UNISpectra.charm} 🍯🏠 Looks like there is nothing in here.`);
                        }
                        const now = Date.now();
                        const elapsed = now - honeyStamp;
                        if (elapsed < timeNeed) {
                            const sLeft = Math.ceil((timeNeed - elapsed) / 1000);
                            return rep.output.reply(`${unisym_js_1.UNISpectra.charm} 🍯🏠 **${String(sLeft).padStart(3, "0")}** Seconds left!`);
                        }
                        const cll = new InventoryEnhanced_1.Collectibles(putCll);
                        cll.raise("honey", rewardHoney);
                        await rep.usersDB.setItem(rep.uid, {
                            honeyStamp: null,
                            collectibles: [...cll],
                        });
                        return rep.output.reply(`${unisym_js_1.UNISpectra.charm} 🍯🏠 ${lineReward.randomValue()}\n\nYou got 🍯**${rewardHoney}**!\n🍯 Your Honey: ${(0, ArielUtils_1.formatValue)(cll.getAmount("honey"), "🍯", true)}`);
                    },
                },
            ];
            const st = {
            ...
            delete st.footer;
            const x = (0, GardenShop_1.GardenChoice)({
                title: str,
                choices,
                style: st,
            });
    ...
    const home = new spectral_home_1.SpectralCMDHome({ isHypen }, [
        {
            key: "shop",
                        ...
                        },
                        {
                            ...
                            async callback(rep2) {
                                const shop = new UTShop({
                                    ...formatShopItems(GardenShop_1.gardenShop, currEvent),
                                    style,
                                });
                                if (isRef) {
                                    shop.resetStocks(...GardenShop_1.gardenShop.itemData
                                        .filter((i) => i.inStock !== false)
                                        .map((i) => i.key));
                                }
            ...
            ? [
                {
                    ...
                    async handler() {
                        const shop = new UTShop({
                            ...formatShopItems({
                                ...GardenShop_1.gardenShop,
                                itemData: GardenShop_1.gardenShop.eventItems,
                            }, currEvent, true),
                            style,
                        });
                        if (isRef) {
                            shop.resetStocks(...GardenShop_1.gardenShop.eventItems
                                .filter((i) => i.inStock !== false)
                                .map((i) => i.key));
                        }
                        await shop.onPlay({ ...ctx, args: [] });
                ...
                const choices = [
                    {
                        ...
                        async callback(rep) {
                            const { gardenBarns = [], money: userMoney = 0 } = await rep.usersDB.getCache(rep.uid);
                            const barns = new Inventory(gardenBarns, GardenConfig_1.CROP_CONFIG.BARN_LIMIT);
                            const canBuy = barns.getAll().filter((i) => !i.isFavorite);
                            if (canBuy.length === 0) {
                                return rep.output.reply(`${unisym_js_1.UNISpectra.charm} 💬 🧑‍🌾 Nothing to buy.`);
                            }
                            let acc = 0;
                            ...
                            return rep.output.reply(`${unisym_js_1.UNISpectra.charm} 💬 🧑‍🌾 Here's your ${(0, ArielUtils_1.formatCash)(acc, true)}\n\n💰 New Balance: ${(0, ArielUtils_1.formatCash)(newMoney)}`);
                        },
                    },
                    {
                        txt: `I want to sell 🫴 this.`,
                        async callback(rep) {
                            const { gardenBarns = [], money: userMoney = 0, gardenHeld = "", } = await rep.usersDB.getCache(rep.uid);
                            const barns = new Inventory(gardenBarns, GardenConfig_1.CROP_CONFIG.BARN_LIMIT);
                            const canBuy = [barns.getOneByID(gardenHeld)]
                                .filter(Boolean)
                                .filter((i) => !i.isFavorite);
                            if (canBuy.length === 0) {
                                return rep.output.reply(`${unisym_js_1.UNISpectra.charm} 💬 🧑‍🌾 I don't see anything.\n\n💡 Hint: try opening your garden barn and hold an item!`);
                            }
                            let acc = 0;
                            for (const item of canBuy) {
                                acc += item.value || 0;
                            ...
                            return rep.output.reply(`${unisym_js_1.UNISpectra.charm} 💬 🧑‍🌾 Here's your ${(0, ArielUtils_1.formatCash)(acc, true)}\n\nSold: ${formatMutationStr(canBuy[0])}\n💰 New Balance: ${(0, ArielUtils_1.formatCash)(newMoney)}`);
                        },
                    },
                    {
                        txt: `How much is 🫴 this worth?`,
                        async callback(rep) {
                            const { gardenBarns = [], gardenHeld = "" } = await rep.usersDB.getCache(rep.uid);
                            const barns = new Inventory(gardenBarns, GardenConfig_1.CROP_CONFIG.BARN_LIMIT);
                            ...
                            return rep.output.reply(`${unisym_js_1.UNISpectra.charm} 💬 🧑‍🌾 I'd value the ${formatMutationStr(canBuy[0])} at ${(0, ArielUtils_1.formatCash)(acc, true)}`);
                        },
                    },
                    {
                        txt: `Nevermind.`,
                        callback(rep) {
                            return rep.output.reply(`${unisym_js_1.UNISpectra.charm} 💬 🧑‍🌾 Goodbye!`);
                        },
                    },
                ];
                const st = {
        ...
        ...(isRelapsed ? relapsedConfig : []),
        {
            ...
            async handler(_, { spectralArgs }) {
                ...
                let plots = new Inventory(rawPlots, plotLimit);
                let seeds = inventory
                    .toUnique()
                    .filter((item) => item.type === "gardenSeed");
                const availablePlots = plotLimit - plots.getAll().length;
                if (availablePlots <= 0) {
                    return output.replyStyled(`🌱 Max plots reached (${plots.getAll().length}/${plotLimit})! Harvest with ${prefix}${commandName}${isHypen ? "-" : " "}harvest or expand with ${prefix}${commandName}${isHypen ? "-" : " "}expand.\n\n` +
                        `**Next Steps**:\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Harvest crops: ${prefix}${commandName}${isHypen ? "-" : " "}harvest\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Expand plot: ${prefix}${commandName}${isHypen ? "-" : " "}expand`, style);
                }
                if (seeds.length === 0) {
                    return output.replyStyled(`🌱 No seeds! Buy some with ${prefix}${commandName}${isHypen ? "-" : " "}shop.\n\n` +
                        `**Next Steps**:\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Visit shop: ${prefix}${commandName}${isHypen ? "-" : " "}shop\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Check items: ${prefix}${commandName}${isHypen ? "-" : " "}list`, style);
                }
                let seed = null;
                let quantity = (0, ArielUtils_1.parseBet)(spectralArgs[1], inventory.getAmount(spectralArgs[0] || "")) || 1;
                quantity = Math.min(quantity, availablePlots, inventory.getAmount(spectralArgs[0] || ""));
                if (spectralArgs[0]) {
                ...
                if (!seed || quantity < 1) {
                    const seedList = seeds
                        .map((s) => `**x${inventory.getAmount(s.key)}** ${s.icon} **${s.name}** (Key: **${s.key}**)`)
                        .join("\n");
                    return output.replyStyled(`❌ Invalid seed key${spectralArgs[0] ? ` "${spectralArgs[0]}"` : ""} or quantity! Use: ${prefix}${commandName}${isHypen ? "-" : " "}plant [seed_key] [quantity]\n\n` +
                        `**Available Seeds**:\n${seedList || "None"}\n\n` +
                        `**Next Steps**:\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} List items: ${prefix}${commandName}${isHypen ? "-" : " "}list\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Buy seeds: ${prefix}${commandName}${isHypen ? "-" : " "}shop`, style);
                }
                const planted = [];
                let firstPlot = null;
                for (let i = 0; i < quantity; i++) {
                    if (inventory.getAmount(seed.key) <= 0 ||
                        plots.getAll().length >= plotLimit)
                        break;
                    inventory.deleteOne(seed.key);
                    const constructions = GardenEventConfig_1.EVENT_CONFIG.EVENTS_CONSTRUCTION.map((i) => (i.shopItems ?? [])).flat();
                    if (constructions.some((i) => i.key === seed.key)) {
                        return output.reply(`🚧 This seed is currently in construction. You **cannot** use it right now.\n\n` +
                            `**Next Steps**:\n` +
                            `${unisym_js_1.UNISpectra.arrowFromT} List items: ${prefix}${commandName}${isHypen ? "-" : " "}list\n` +
                            `${unisym_js_1.UNISpectra.arrowFromT} Buy seeds: ${prefix}${commandName}${isHypen ? "-" : " "}shop`);
                    }
                    const allItems = GardenEventConfig_1.EVENT_CONFIG.allItems;
                    const shopItem = allItems.find((i) => seed.key === i?.key);
                    const priceInt = shopItem?.price ?? seed.cropData.baseValue;
                    const price = shopItem.priceType !== "money" && shopItem.priceType
                        ? 0
                        : Math.min(seed.cropData.baseValue || 0, priceInt / (seed.cropData.harvests || 1));
                    let plot = {
                        ...
                        lastMutation: null,
                        kiloGrams: 0,
                        maxKiloGrams: (0, unitypes_1.randomBiased)(GardenConfig_1.CROP_CONFIG.MIN_KG + (seed.cropData.baseKG ?? 0), GardenConfig_1.CROP_CONFIG.MAX_KG, GardenConfig_1.CROP_CONFIG.KILO_BIAS),
                        mutationAttempts: 0,
                        baseKiloGrams: seed.cropData.baseKG ?? 0,
                        lastUpdated: Date.now(),
                    };
                    if (Math.random() < 0.1) {
                        plot = await applyMutation(plot, new Inventory(rawInventory.filter((item) => item.type === "gardenTool")), exiPets, false, currWeather);
                    }
                    plot.mutationAttempts = 0;
                    plot = await autoUpdateCropData(plot, new Inventory(rawInventory.filter((item) => item.type === "gardenTool")), exiPets);
                    firstPlot ??= plot;
                    plots.addOne(plot);
                    ...
                    planted.push(formatMutationStr({ ...plot, name: seed.name, icon: seed.icon }));
                }
                await money.setItem(input.senderID, {
                ...
                return ctx.output.replyStyled(`🌱 Planted ${planted.length} seed${planted.length !== 1 ? "s" : ""} (${plots.getAll().length}/${plotLimit} plots):\n${planted.join("\n")}\n` +
                    `⏳ First ready in: ${(0, ArielUtils_1.formatTimeSentence)(cropTimeLeft(firstPlot)) ||
                        "***ALREADY READY!***"}\n\n` +
                    `**Next Steps**:\n` +
                    `${unisym_js_1.UNISpectra.arrowFromT} Check plots: ${prefix}${commandName}${isHypen ? "-" : " "}plots\n` +
                    `${unisym_js_1.UNISpectra.arrowFromT} Harvest later: ${prefix}${commandName}${isHypen ? "-" : " "}harvest`, style);
            },
        },
        {
            key: "collect",
            ...
            async handler(_, { spectralArgs }) {
                ...
                const plots = new Inventory(rawPlots, plotLimit);
                const barn = new Inventory(rawBarns, GardenConfig_1.CROP_CONFIG.BARN_LIMIT);
                let inventory = new Inventory(rawInventory);
                let moneyEarned = 0;
                const harvested = [];
                const seedsGained = [];
                const tools = new Inventory(rawInventory.filter((item) => item.type === "gardenTool"));
                const sortedPlots = [...plots].sort((a, b) => calculateCropValue(b, inflationRate).allYield -
                    calculateCropValue(a, inflationRate).allYield);
                const type = spectralArgs[0];
                const isAll = type === "all";
                if ((!type || !sortedPlots.some((i) => i.seedKey === type)) && !isAll) {
                    const plotList = Datum_1.Datum.toUniqueArray(sortedPlots, (i) => i.seedKey)
                        .filter(isCropReady)
                        .map((s) => `**x${plots.getAll().filter((i) => i.seedKey === s.seedKey).length}** ${s.icon} **${s.name}** (Key: **${s.seedKey}**)`)
                        .join("\n");
                    return output.replyStyled(`🌱 Please specify "all" or a seed key to collect.\n\n` +
                        `**Plots**:\n${plotList || "None"}\n\n` +
                        `**Next Steps**:\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} List plots: ${prefix}${commandName}${isHypen ? "-" : " "}plots\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Buy seeds: ${prefix}${commandName}${isHypen ? "-" : " "}shop`, style);
                }
                const readyPlots = [];
                ...
                if (readyPlots.length === 0) {
                    return output.replyStyled(`🌱 No crops ready! Check plots with ${prefix}${commandName}${isHypen ? "-" : " "}plots.\n\n` +
                        `**Next Steps**:\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} View plots: ${prefix}${commandName}${isHypen ? "-" : " "}plots\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Plant seeds: ${prefix}${commandName}${isHypen ? "-" : " "}plant`, style);
                }
                if (barn.isFull()) {
                    return output.replyStyled(`🌱 Barn is full! Check barn items with ${prefix}${commandName}${isHypen ? "-" : " "}barn.\n\n` +
                        `**Next Steps**:\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} View plots: ${prefix}${commandName}${isHypen ? "-" : " "}plots\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Plant seeds: ${prefix}${commandName}${isHypen ? "-" : " "}plant`, style);
                }
                for (const plot of readyPlots) {
                    await autoUpdateCropData(plot, new Inventory(rawInventory.filter((item) => item.type === "gardenTool")), exiPets);
                    if (plot.mutation.length > 0) {
                        gardenStats.mutationsFound =
                            (gardenStats.mutationsFound || 0) + plot.mutation.length;
                    }
                    ...
                    let yields = Math.min(value.yields, GardenConfig_1.CROP_CONFIG.BARN_LIMIT - barn.size());
                    ...
                    if (Math.random() < GardenConfig_1.CROP_CONFIG.LUCKY_HARVEST_CHANCE) {
                        const shopItem = [
                            ...GardenShop_1.gardenShop.itemData,
                            ...GardenShop_1.gardenShop.eventItems,
                    ...
                    if (plot.harvestsLeft <= 0) {
                        ...
                        plot.maxKiloGrams = (0, unitypes_1.randomBiased)(GardenConfig_1.CROP_CONFIG.MIN_KG, GardenConfig_1.CROP_CONFIG.MAX_KG, GardenConfig_1.CROP_CONFIG.KILO_BIAS);
                        plot.kiloGrams = 0;
                        if (Math.random() < 0.1) {
                            await applyMutation(plot, new Inventory(rawInventory.filter((item) => item.type === "gardenTool")), exiPets, false, currWeather);
                        }
                        plot.mutationAttempts = 0;
                        plots.deleteByID(plot.uuid);
                        plots.addOne(plot);
                ...
                const harvestedStr = [...harvested]
                    ...
                    .map(({ plot, value }) => `(x${value.yields}) ${(plot.mutation ?? []).length > 0
                    ? `[ ${plot.mutation
                        .map((i) => `${getMutation(i)?.icon ?? "❓"} ${fonts.double_struck(i)}`)
                        .join(" + ")} ] `
                    : ""}${plot.icon} ${plot.name} (${plot.kiloGrams}kg)`);
                const addedEarns = gardenEarns - origEarns;
                return output.replyStyled(`✅🧺 **Collected and added to the Barn! (${barn.size()}/${barn.limit})**:\n${harvestedStr.join("\n")}${barn.isFull() ? `\n🏡 Barn is full!` : ""}\n\n` +
                    (seedsGained.length > 0
                        ? `🌱🧺 **Lucky Harvest Seeds**:\n${seedsGained.join("\n")}\n\n`
                        : "") +
                    // `💰 Earned: ${formatCash(moneyEarned, true)}\n` +
                    // `💵 Balance: ${formatCash(userMoney + moneyEarned)}\n\n` +
                    `📈 Total Earns: **${(0, ArielUtils_1.formatCash)(gardenEarns)}** (+${(0, ArielUtils_1.abbreviateNumber)(addedEarns)})\n\n` +
                    `**Next Steps**:\n` +
                    `${unisym_js_1.UNISpectra.arrowFromT} Sell crops: ${prefix}${commandName}${isHypen ? "-" : " "}sell\n` +
                    `${unisym_js_1.UNISpectra.arrowFromT} Check the barn: ${prefix}${commandName}${isHypen ? "-" : " "}barn`, style);
            },
        },
        {
            key: "plots",
            ...
            async handler(_, { spectralArgs }) {
                ...
                const type = `${spectralArgs[0] || "least-time"}`.toLowerCase();
                if (!isNaN(parseInt(spectralArgs[0])) && spectralArgs[0]) {
                    return output.reply(`🌱 Please use the page number as second argument. For the first argument, choose from: **${plotSorts.join(", ")}**`);
                }
                if (!plotSorts.includes(type)) {
                    return output.reply(`🌱 Invalid sorting type as first argument. Choose from: **${plotSorts.join(", ")}**`);
                }
                const start = (page - 1) * exports.ITEMS_PER_PAGE;
                const end = page * exports.ITEMS_PER_PAGE;
                for (const plot of plots) {
                    await autoUpdateCropData(plot, new Inventory(rawInventory.filter((item) => item.type === "gardenTool")), exiPets);
                }
                ...
                let result = `🌱 **${name}'s Garden Plots (${plots.getAll().length}/${plotLimit}, Page ${page})**:\n\n`;
                if (sortedPlots.length === 0) {
                    return output.replyStyled(`🌱 No plots! Plant seeds with ${prefix}${commandName}${isHypen ? "-" : " "}plant.\n\n` +
                        `**Next Steps**:\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Plant seeds: ${prefix}${commandName}${isHypen ? "-" : " "}plant\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Buy seeds: ${prefix}${commandName}${isHypen ? "-" : " "}shop`, style);
                }
                for (let [, { num, plot }] of sortedPlots.entries()) {
                    ...
                    result +=
                        `${num}. ${formatMutationStr(plot)} (x${calc.yields})\n` +
                            `${unisym_js_1.UNIRedux.charm} Harvests Left: ${plot.harvestsLeft}\n` +
                            `${unisym_js_1.UNIRedux.charm} Time Left: ${(0, ArielUtils_1.formatTimeSentence)(timeLeft) ||
                                (!isCropReady(plot) ? "***BUGGED***!" : "***READY***!")}\n\n`;
                }
                if (plots.getAll().length > end) {
                    result += `View more: ${prefix}${commandName}${isHypen ? "-" : " "}plots ${type} ${page + 1}\n`;
                }
                result += `💡🏗️ To Shovel: ***Reply with***:\nshovel <number> <number> <number>\n`;
                result += `\n📈 Total Earns: ${(0, ArielUtils_1.formatCash)(gardenEarns, true)}\n\n`;
                result +=
                    `**Next Steps**:\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Harvest crops: ${prefix}${commandName}${isHypen ? "-" : " "}harvest\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Favorite crops: ${prefix}${commandName}${isHypen ? "-" : " "}plots`;
                await money.setItem(input.senderID, {
                ...
                info.atReply(async (rep) => {
                    ...
                    const { gardenPlots: rawPlots = [] } = await rep.usersDB.getCache(rep.uid);
                    const plots = new Inventory(rawPlots);
                    const targets = nums
        ...
        },
        {
            ...
            async handler(_, { spectralArgs }) {
                ...
                const uniqueBarns = barn
                    .toUnique((i) => `${gardenHeld === i.uuid ? `🫴 ` : ""}${formatMutationStr(i)}`)
                    .map((i) => ({
                    item: i,
                    amount: barn
                        .getAll()
                        .filter((j) => `${gardenHeld === j.uuid ? `🫴 ` : ""}${formatMutationStr(j)}` ===
                        `${gardenHeld === i.uuid ? `🫴 ` : ""}${formatMutationStr(i)}`).length,
                }));
                const sortedBarns = [...uniqueBarns]
                    .sort((a, b) => {
                    if (a.item.uuid === gardenHeld)
                        return -1;
                    if (b.item.uuid === gardenHeld)
                        return 1;
                    const aScore = a.item.value + (a.item.isFavorite ? 1_000_000_000_000 : 0);
                    const bScore = b.item.value + (b.item.isFavorite ? 1_000_000_000_000 : 0);
                ...
                let result = `🏡 **${name}'s Barn (${barn.getAll().length}/${GardenConfig_1.CROP_CONFIG.BARN_LIMIT}, Page ${page})**:\n\n`;
                if (sortedBarns.length === 0) {
                    return output.replyStyled(`🏡 No barn items! Plant and collect crops with ${prefix}${commandName}${isHypen ? "-" : " "}collect.\n\n` +
                        `**Next Steps**:\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Plant seeds: ${prefix}${commandName}${isHypen ? "-" : " "}plant\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Collect crops: ${prefix}${commandName}${isHypen ? "-" : " "}collect`, style);
                }
                for (let [, { num, item, amount }] of sortedBarns.entries()) {
                    result += `${num}. ${amount > 1 ? `[**x${amount}**] ` : ""}${gardenHeld === item.uuid ? `🫴 ` : ""}${formatMutationStr(item)}\n\n`;
                }
                if (barn.getAll().length > end) {
                    result += `View more: ${prefix}${commandName}${isHypen ? "-" : " "}barn ${page + 1}\n`;
                }
                result += `💡🫴 Options: ***Reply with***\nhold <number>\nfavorite <number> [number] [number]\nunfavorite <number> [number] [number]`;
                result += `\n📈 Total Earns: ${(0, ArielUtils_1.formatCash)(gardenEarns, true)}\n\n`;
                result +=
                    `**Next Steps**:\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Collect crops: ${prefix}${commandName}${isHypen ? "-" : " "}collect\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Sell barn items: ${prefix}${commandName}${isHypen ? "-" : " "}sell`;
                const info = await output.replyStyled(result, style);
                info.atReply(async (rep) => {
                    if (rep.uid !== input.sid) {
                        return;
                    ...
                    type = type.toLowerCase();
                    const nums = nums_.map(Number);
                    const { gardenBarns: rawBarns = [], gardenHeld = "" } = await rep.usersDB.getCache(rep.uid);
                    const barn = new Inventory(rawBarns);
                    const targets = nums
                    ...
                    if (type === "hold") {
                        ...
                        let str = `✅🫴 **${isUnheld ? "Unheld" : "Held"} Successfully!**\n\n`;
                        str += `${formatMutationStr(target)}\n`;
                        str +=
                            `\n**Next Steps**:\n` +
                                `${unisym_js_1.UNISpectra.arrowFromT} Sell the held item: ${prefix}${commandName}${isHypen ? "-" : " "}sell\n` +
                                `${unisym_js_1.UNISpectra.arrowFromT} Favorite crops: ${prefix}${commandName}${isHypen ? "-" : " "}plots`;
                        await rep.usersDB.setItem(rep.uid, {
                        ...
                        let str = `✅💖 **${type === "favorite" ? "Favorited" : "Unfavorited"} Successfully!**\n\n`;
                        for (const target of targets) {
                            if (type === "favorite" && target.isFavorite)
                                continue;
                            if (type === "unfavorite" && !target.isFavorite)
                                continue;
                        ...
                        str +=
                            `\n**Next Steps**:\n` +
                                `${unisym_js_1.UNISpectra.arrowFromT} Sell crops: ${prefix}${commandName}${isHypen ? "-" : " "}sell\n` +
                                `${unisym_js_1.UNISpectra.arrowFromT} Check barn: ${prefix}${commandName}${isHypen ? "-" : " "}barn`;
                        await rep.usersDB.setItem(rep.uid, {
                            gardenBarns: Array.from(barn),
                        });
                        return rep.output.reply(str);
        ...
        {
            key: "top",
            description: "View top 10 garden earners (paged, ranks 20-11 for page 2, etc.)",
            aliases: ["-t"],
            args: ["[page]"],
            icon: "🏆",
            async handler(_, { spectralArgs }) {
                ...
                for (const user of Object.values(allUsers)) {
                    if ((user.gardenPlots ?? []).length === 0 ||
                        (user.gardenEarns ?? 0) < 1) {
                        continue;
                    }
                    userStats.push({
                        userId: user.userID,
                ...
                const sortedUsers = userStats.sort((a, b) => b.totalEarns - a.totalEarns);
                const currentPageUsers = sortedUsers.slice(startRank - 1, endRank);
                let result = `🏆 **TOP ${endRank} GARDENERS**\n${unisym_js_1.UNISpectra.arrowFromT} Page **${page}**:\n\n`;
                if (currentPageUsers.length === 0) {
                    return output.replyStyled(`🌱 No users found for ranks ${startRank}-${endRank}!\n\n` +
                        `**Next Steps**:\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Check another page: ${prefix}${commandName}${isHypen ? "-" : " "}top [page]\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Harvest crops: ${prefix}${commandName}${isHypen ? "-" : " "}harvest`, style);
                }
                currentPageUsers.forEach((user, index) => {
                    result +=
                        `━━━━━━ ${startRank + index} ━━━━━━\n` +
                            `${unisym_js_1.UNIRedux.arrowFromT}  **${user.name}**${user?.user.userMeta?.name &&
                                user.name !== user?.user.userMeta?.name
                                ? ` (${user.user.userMeta.name})`
                                : ""}\n` +
                            `📊🌱 ${(0, ArielUtils_1.formatCash)(user.totalEarns, false)}\n`;
                });
                if (sortedUsers.length > endRank) {
                    result += `${unisym_js_1.UNIRedux.standardLine}\nView more: ${prefix}${commandName}${isHypen ? "-" : " "}top ${page + 1}\n`;
                }
                if (page > 2) {
                    result += `View previous: ${prefix}${commandName}${isHypen ? "-" : " "}top ${page - 1}\n`;
                }
                result +=
                    `\n**Next Steps**:\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Harvest crops: ${prefix}${commandName}${isHypen ? "-" : " "}harvest\n` +
        ...
        },
        {
            ...
            async handler(_, { spectralArgs }) {
                ...
                const gardenItems = inventory
                    .toUnique()
                    .filter((item) => ["gardenSeed", "gardenPetCage", "gardenTool"].includes(item.type));
                const page = parseInt(spectralArgs[0]) || 1;
                const start = (page - 1) * exports.ITEMS_PER_PAGE;
                const end = page * exports.ITEMS_PER_PAGE;
                ...
                if (currentItems.length === 0) {
                    return output.replyStyled(`🌱 No garden items! Buy some with ${prefix}${commandName}${isHypen ? "-" : " "}shop.\n\n` +
                        `**Next Steps**:\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Visit shop: ${prefix}${commandName}${isHypen ? "-" : " "}shop\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Plant seeds: ${prefix}${commandName}${isHypen ? "-" : " "}plant`, style);
                }
                currentItems.forEach((_item, index) => {
                    ...
                    result +=
                        `${start + index + 1}. ${item.icon} **${item.name}**${count > 1 ? ` (x${count})` : ""}${inventory.get(item.key).some((i) => i.isFavorite) ? ` 💖` : ""}\n` +
                            `${unisym_js_1.UNIRedux.charm} Type: ${item.type}\n` +
                            `${unisym_js_1.UNIRedux.charm} Key: **${item.key}**\n` +
                            `${item.type === "gardenSeed"
                                ? `${unisym_js_1.UNIRedux.charm} Base Value: ${(0, ArielUtils_1.formatCash)(item.cropData.baseValue, true)}`
                                : `${unisym_js_1.UNIRedux.charm} Sell Price: ${(0, ArielUtils_1.formatCash)(item.sellPrice)}`}\n` +
                            (item.type === "gardenPetCage"
                                ? `${unisym_js_1.UNIRedux.charm} Collects: ${item.petData.seedTypes.join(", ")} (${item.petData.collectionRate}/min)\n`
                                : "") +
                            (item.type === "gardenTool"
                                ? `${unisym_js_1.UNIRedux.charm} Effect: ${item.toolData.favoriteEnabled
                                    ? "Enables favoriting"
                                    : `Growth ${item.toolData.growthMultiplier || 1}x, Mutations (${Object.keys(item.toolData.mutationChance || {}).join(", ")}) +${Object.values(item.toolData.mutationChance || {}).reduce((a, b) => a + b, 0) * 100}%`}\n`
                                : "") +
                            `\n`;
                });
                if (gardenItems.length > end) {
                    result += `View more: ${prefix}${commandName}${isHypen ? "-" : " "}list ${page + 1}\n`;
                }
                ...
                result +=
                    `**Next Steps**:\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Plant seeds: ${prefix}${commandName}${isHypen ? "-" : " "}plant\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Favorite items: ${prefix}${commandName}${isHypen ? "-" : " "}plots`;
                return output.replyStyled(result, style);
            },
        },
        {
            ...
            async handler(_, { spectralArgs }) {
                ...
                if (userGems < stealCost) {
                    return output.replyStyled(`❌ You need ${(0, ArielUtils_1.formatValue)(stealCost, "💎", true)} to steal a crop! Your gems: ${(0, ArielUtils_1.formatValue)(userGems, "💎", true)}.\n\n` +
                        `**Next Steps**:\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Sell crops: ${prefix}${commandName}${isHypen ? "-" : " "}sell\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Harvest crops: ${prefix}${commandName}${isHypen ? "-" : " "}harvest`, style);
                }
                ...
                if (!UID || UID === input.sid) {
                    return output.replyStyled(`❌ Please specify a player ID to steal from or reply to their message, or mention them!\n\n` +
                        `**Next Steps**:\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Try again: ${prefix}${commandName}${isHypen ? "-" : " "}steal [player_id]`, style);
                }
                const target = await money.getCache(UID);
                const targetPlots = new Inventory(target.gardenPlots || []);
                ...
                if (stealablePlots.length === 0) {
                    return output.replyStyled(`❌ No stealable crops in that player's garden!\n\n` +
                        `**Next Steps**:\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Try another player: ${prefix}${commandName}${isHypen ? "-" : " "}steal [player_id]`, style);
                }
                const sortedPlots = stealablePlots.toSorted((a, b) => calculateCropValue(b, inflationRate).allYield -
                    calculateCropValue(a, inflationRate).allYield);
                const stolenPlot = sortedPlots[0];
                const stealSuccess = Math.random() < 0.3;
                if (!stealSuccess) {
                    await money.setItem(input.senderID, { money: userMoney + 100 });
                    return output.replyStyled(`❌ Steal failed for ${(0, ArielUtils_1.formatValue)(stealCost, "💎", true)}! You received ${(0, ArielUtils_1.formatCash)(100)} as compensation.\n\n` +
                        `**Next Steps**:\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Try again: ${prefix}${commandName}${isHypen ? "-" : " "}steal [player_id]\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Harvest crops: ${prefix}${commandName}${isHypen ? "-" : " "}harvest`, style);
                }
                const myPlots = new Inventory(rawPlots);
                const availablePlots = plotLimit - myPlots.getAll().length;
                if (availablePlots <= 0) {
                    return output.replyStyled(`🌱 Max plots reached (${myPlots.getAll().length}/${plotLimit})! Harvest with ${prefix}${commandName}${isHypen ? "-" : " "}harvest or expand with ${prefix}${commandName}${isHypen ? "-" : " "}expand.\n\n` +
                        `**Next Steps**:\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Harvest crops: ${prefix}${commandName}${isHypen ? "-" : " "}harvest\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Expand plot: ${prefix}${commandName}${isHypen ? "-" : " "}expand`, style);
                }
                ...
                return output.replyStyled(`✅ Stole ${formatMutationStr(stolenPlot)} for ${(0, ArielUtils_1.formatValue)(stealCost, "💎", true)}! It was added to your plots!\n\n${unisym_js_1.UNISpectra.charm} Total Value: ${(0, ArielUtils_1.formatCash)(calculateCropValue(stolenPlot, inflationRate).allYield, true)}\n\n` +
                    `**Next Steps**:\n` +
                    `${unisym_js_1.UNISpectra.arrowFromT} Check items: ${prefix}${commandName}${isHypen ? "-" : " "}list\n` +
                    `${unisym_js_1.UNISpectra.arrowFromT} Plant seeds: ${prefix}${commandName}${isHypen ? "-" : " "}plant`, style);
            },
        },
        {
            ...
            async handler(_, { spectralArgs }) {
                ...
                if (!currentHeld || !xBarn.hasByID(gardenHeld)) {
                    return output.replyStyled(`❌ No item held! Hold an item with ${prefix}${commandName}${isHypen ? "-" : " "}barn.\n\n` +
                        `**Next Steps**:\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Check barn: ${prefix}${commandName}${isHypen ? "-" : " "}barn\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Collect crops: ${prefix}${commandName}${isHypen ? "-" : " "}collect`, style);
                }
                if (currentHeld.isFavorite) {
                    return output.replyStyled(`❌ Cannot gift favorited item ${formatMutationStr(currentHeld)}!\n\n` +
                        `**Next Steps**:\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Unfavorite item: ${prefix}${commandName}${isHypen ? "-" : " "}plots\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Check barn: ${prefix}${commandName}${isHypen ? "-" : " "}barn`, style);
                }
                const targetUID = spectralArgs[0] || input.detectID;
                if (!targetUID) {
                    return output.replyStyled(`❌ Please specify a player ID to gift to, reply to their message, or mention them!\n\n` +
                        `**Next Steps**:\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Try again: ${prefix}${commandName}${isHypen ? "-" : " "}gift [player_id]`, style);
                }
                if (targetUID === input.sid) {
                    return output.replyStyled(`❌ You cannot gift to yourself!\n\n` +
                        `**Next Steps**:\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Try again: ${prefix}${commandName}${isHypen ? "-" : " "}gift [player_id]\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Check barn: ${prefix}${commandName}${isHypen ? "-" : " "}barn`, style);
                }
                const target = await money.getCache(targetUID);
                if (!target || !target.name) {
                    return output.replyStyled(`❌ Player with ID ${targetUID} not found!\n\n` +
                        `**Next Steps**:\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Try another player: ${prefix}${commandName}${isHypen ? "-" : " "}gift [player_id]`, style);
                }
                const targetBarn = new Inventory(target.gardenBarns || [], GardenConfig_1.CROP_CONFIG.BARN_LIMIT);
                if (targetBarn.isFull()) {
                    return output.replyStyled(`❌ ${target.name}'s barn is full (${targetBarn.size()}/${GardenConfig_1.CROP_CONFIG.BARN_LIMIT})!\n\n` +
                        `**Next Steps**:\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Try another player: ${prefix}${commandName}${isHypen ? "-" : " "}gift [player_id]\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Check barn: ${prefix}${commandName}${isHypen ? "-" : " "}barn`, style);
                }
                ...
                return output.replyStyled(`🎁 Successfully gifted ${formatMutationStr(currentHeld)} to **${target.name}**!\n\n` +
                    `**Next Steps**:\n` +
                    `${unisym_js_1.UNISpectra.arrowFromT} Check barn: ${prefix}${commandName}${isHypen ? "-" : " "}barn\n` +
                    `${unisym_js_1.UNISpectra.arrowFromT} Collect more crops: ${prefix}${commandName}${isHypen ? "-" : " "}collect`, style);
            },
        },
        {
            key: "growall",
            ...
            async handler() {
                ...
                if (userGems < growAllCost) {
                    return output.replyStyled(`❌ You need ${(0, ArielUtils_1.formatValue)(growAllCost, "💎", true)} to grow all crops! Your gems: ${(0, ArielUtils_1.formatValue)(userGems, "💎", true)}.\n\n` +
                        `**Next Steps**:\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Sell crops: ${prefix}${commandName}${isHypen ? "-" : " "}sell\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Harvest crops: ${prefix}${commandName}${isHypen ? "-" : " "}harvest`, style);
                }
                ...
                if (plots.getAll().length === 0) {
                    return output.replyStyled(`🌱 No crops to grow! Plant seeds with ${prefix}${commandName}${isHypen ? "-" : " "}plant.\n\n` +
                        `**Next Steps**:\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Plant seeds: ${prefix}${commandName}${isHypen ? "-" : " "}plant\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Buy seeds: ${prefix}${commandName}${isHypen ? "-" : " "}shop`, style);
                }
                ...
                return output.replyStyled(`🌱 All crops grown instantly for ${(0, ArielUtils_1.formatValue)(growAllCost, "💎", true)}!\n\n` +
                    `**Next Steps**:\n` +
                    `${unisym_js_1.UNISpectra.arrowFromT} Harvest crops: ${prefix}${commandName}${isHypen ? "-" : " "}harvest\n` +
                    `${unisym_js_1.UNISpectra.arrowFromT} Check plots: ${prefix}${commandName}${isHypen ? "-" : " "}plots`, style);
            },
        },
        {
            key: "expand",
            ...
            async handler(_, { spectralArgs }) {
                ...
                const currentTime = Date.now();
                if (plotLimit >= exports.PLOT_EXPANSION_LIMIT) {
                    return output.replyStyled(`🌱 Max plot limit (${exports.PLOT_EXPANSION_LIMIT}) reached!\n\n` +
                        `**Next Steps**:\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Harvest crops: ${prefix}${commandName}${isHypen ? "-" : " "}harvest\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Plant seeds: ${prefix}${commandName}${isHypen ? "-" : " "}plant`, style);
                }
                ...
                if (!expansionType) {
                    return output.replyStyled(`❌ Invalid expansion type! Use: side, rear1, or rear2.\n\n` +
                        `💰 **Costs**:\n\nSide - ${(0, ArielUtils_1.formatCash)(sideExpansionCost, true)}\nRear1 - ${(0, ArielUtils_1.formatCash)(rearExpansion1Cost, true)}\nRear2 - ${(0, ArielUtils_1.formatCash)(rearExpansion2Cost, true)}\n\n` +
                        `**Next Steps**:\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Try again: ${prefix}${commandName}${isHypen ? "-" : " "}expand [side/rear1/rear2]`, style);
                }
                if (expansionType === "side") {
                    if (userMoney < sideExpansionCost) {
                        return output.replyStyled(`❌ You need ${(0, ArielUtils_1.formatCash)(sideExpansionCost)} for a side expansion! Your balance: ${(0, ArielUtils_1.formatCash)(userMoney)}.\n\n` +
                            `**Next Steps**:\n` +
                            `${unisym_js_1.UNISpectra.arrowFromT} Sell crops: ${prefix}${commandName}${isHypen ? "-" : " "}sell\n` +
                            `${unisym_js_1.UNISpectra.arrowFromT} Harvest crops: ${prefix}${commandName}${isHypen ? "-" : " "}harvest`, style);
                    }
                    if (currentTime - lastSideExpansion < sideExpansionDelay) {
                        return output.replyStyled(`⏳ Side expansion on cooldown! Try again in ${(0, ArielUtils_1.formatTimeSentence)(lastSideExpansion + sideExpansionDelay - currentTime) || "Now?"}.\n\n` +
                            `**Next Steps**:\n` +
                            `${unisym_js_1.UNISpectra.arrowFromT} Harvest crops: ${prefix}${commandName}${isHypen ? "-" : " "}harvest`, style);
                    }
                    await money.setItem(input.senderID, {
                    ...
                    return output.replyStyled(`🌱 Plot expanded by ${sideExpansionPlots} slots for ${(0, ArielUtils_1.formatCash)(sideExpansionCost)}!\n\n` +
                        `**Next Steps**:\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Plant seeds: ${prefix}${commandName}${isHypen ? "-" : " "}plant\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Check plots: ${prefix}${commandName}${isHypen ? "-" : " "}plots`, style);
                }
                    ...
                    if (userMoney < rearExpansion1Cost) {
                        return output.replyStyled(`❌ You need ${(0, ArielUtils_1.formatCash)(rearExpansion1Cost)} for the first rear expansion! Your balance: ${(0, ArielUtils_1.formatCash)(userMoney)}.\n\n` +
                            `**Next Steps**:\n` +
                            `${unisym_js_1.UNISpectra.arrowFromT} Sell crops: ${prefix}${commandName}${isHypen ? "-" : " "}sell\n` +
                            `${unisym_js_1.UNISpectra.arrowFromT} Harvest crops: ${prefix}${commandName}${isHypen ? "-" : " "}harvest`, style);
                    }
                    if (currentTime - lastRearExpansion1 < rearExpansion1Delay) {
                        return output.replyStyled(`⏳ First rear expansion on cooldown! Try again in ${(0, ArielUtils_1.formatTimeSentence)(lastRearExpansion1 + rearExpansion1Delay - currentTime) || "Now?"}.\n\n` +
                            `**Next Steps**:\n` +
                            `${unisym_js_1.UNISpectra.arrowFromT} Harvest crops: ${prefix}${commandName}${isHypen ? "-" : " "}harvest`, style);
                    }
                    await money.setItem(input.senderID, {
                    ...
                    return output.replyStyled(`🌱 Plot expanded by ${rearExpansionPlots} slots for ${(0, ArielUtils_1.formatCash)(rearExpansion1Cost)}!\n\n` +
                        `**Next Steps**:\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Plant seeds: ${prefix}${commandName}${isHypen ? "-" : " "}plant\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Check plots: ${prefix}${commandName}${isHypen ? "-" : " "}plots`, style);
                }
                    ...
                    if (userMoney < rearExpansion2Cost) {
                        return output.replyStyled(`❌ You need ${(0, ArielUtils_1.formatCash)(rearExpansion2Cost)} for the second rear expansion! Your balance: ${(0, ArielUtils_1.formatCash)(userMoney)}.\n\n` +
                            `**Next Steps**:\n` +
                            `${unisym_js_1.UNISpectra.arrowFromT} Sell crops: ${prefix}${commandName}${isHypen ? "-" : " "}sell\n` +
                            `${unisym_js_1.UNISpectra.arrowFromT} Harvest crops: ${prefix}${commandName}${isHypen ? "-" : " "}harvest`, style);
                    }
                    if (currentTime - lastRearExpansion2 < rearExpansion2Delay) {
                        return output.replyStyled(`⏳ Second rear expansion on cooldown! Try again in ${(0, ArielUtils_1.formatTimeSentence)(lastRearExpansion2 + rearExpansion2Delay - currentTime) || "Now?"}.\n\n` +
                            `**Next Steps**:\n` +
                            `${unisym_js_1.UNISpectra.arrowFromT} Harvest crops: ${prefix}${commandName}${isHypen ? "-" : " "}harvest`, style);
                    }
                    await money.setItem(input.senderID, {
                    ...
                    return output.replyStyled(`🌱 Plot expanded by ${rearExpansionPlots} slots for ${(0, ArielUtils_1.formatCash)(rearExpansion2Cost)}!\n\n` +
                        `**Next Steps**:\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Plant seeds: ${prefix}${commandName}${isHypen ? "-" : " "}plant\n` +
                        `${unisym_js_1.UNISpectra.arrowFromT} Check plots: ${prefix}${commandName}${isHypen ? "-" : " "}plots`, style);
                }
        ...
        },
        {
            ...
            async handler() {
                ...
                const result = [
                    ...
                    `${unisym_js_1.UNIRedux.charm} Growth Speed: ${weather.growthMultiplier}x`,
                ];
                for (const effect of weather.effects ?? []) {
                    if (effect.mutationChance != null) {
                        result.push(`${unisym_js_1.UNIRedux.charm} Mutation Chance: +${(effect.mutationChance * 100).toFixed(0)}%`);
                    }
                    if (effect.mutationType) {
                        result.push(`${unisym_js_1.UNIRedux.charm} Mutation Type: ${effect.mutationType}`);
                    }
                }
                result.push(`🕒 Next Weather in: ${(0, ArielUtils_1.formatTimeSentence)(timeLeft) || "Ready!"}`);
                result.push(`\n🌟 **Upcoming Weather:**\n${upcomingEvents.join("\n")}`);
                result.push(`\n**Next Steps**:\n` +
                    `${unisym_js_1.UNISpectra.arrowFromT} Buy event seeds: ${prefix}${commandName}${isHypen ? "-" : " "}shop\n` +
                    `${unisym_js_1.UNISpectra.arrowFromT} Plant seeds: ${prefix}${commandName}${isHypen ? "-" : " "}plant`);
        ...
        },
        {
            ...
            args: ["[number/'reset']"],
            async handler(_, { spectralArgs }) {
                if (!input.hasRole(InputClass_1.InputRoles.MODERATORBOT)) {
        ...
        },
        {
            ...
            async handler(_, { spectralArgs }) {
                ...
                if (isNaN(parseInt(spectralArgs[0]))) {
                    ...
                    if (target) {
                        const bres = (0, GardenBalancer_1.evaluateItemBalance)(target);
                        if (bres) {
                            const sortedItems = allItems
                                .toSorted((a, b) => ((0, GardenBalancer_1.evaluateItemBalance)(b)?.score || 0) -
                                ((0, GardenBalancer_1.evaluateItemBalance)(a)?.score || 0))
                                .filter((i) => (0, GardenBalancer_1.evaluateItemBalance)(i) !== null);
                            const itemIndex = sortedItems.findIndex((i) => i.key === key);
                            const foundPage = itemIndex === -1
                                ? 1
                                : Math.floor(itemIndex / exports.ITEMS_PER_PAGE) + 1;
                            const topNumber = itemIndex + 1;
                            return output.replyStyled(`Found in **Page**: ${foundPage}\n#${topNumber}. ${bres.item.icon} **${bres.item.name}** (${bres.item.key})\n🏅 **SCORE**: ${bres.score}\n**Stock Chance**: ${(bres.stockChance * 100).toFixed(2)}%\n🛒 ${(0, ArielUtils_1.formatCash)(bres.price, true)}\n🪙 ${(0, ArielUtils_1.abbreviateNumber)(bres.item.cropData.baseValue || 0)} | 🧺 ${(0, ArielUtils_1.abbreviateNumber)(bres.item.cropData.harvests || 0)} | ⏳ ${(0, ArielUtils_1.formatTimeSentence)(bres.item.cropData.growthTime || 0) ||
                                "Instant"}\n\n`, style);
                        }
                    }
                }
                ...
                for (const item of currentItems) {
                    const bres = (0, GardenBalancer_1.evaluateItemBalance)(item);
                    const i = sortedItems.findIndex((i) => i.key === item.key) + 1;
                    result += `${i}. ${bres.item.icon} **${bres.item.name}** (${bres.item.key})\n🏅 **SCORE**: ${bres.score}\n**Stock Chance**: ${(bres.stockChance * 100).toFixed(2)}%\n🛒 ${(0, ArielUtils_1.formatCash)(bres.price, true)}\n🪙 ${(0, ArielUtils_1.abbreviateNumber)(bres.item.cropData.baseValue || 0)} | 🧺 ${(0, ArielUtils_1.abbreviateNumber)(bres.item.cropData.harvests || 0)} | ⏳ ${(0, ArielUtils_1.formatTimeSentence)(bres.item.cropData.growthTime || 0) || "Instant"}\n\n`;
                }
                result += `${unisym_js_1.UNIRedux.arrowFromT} Next page: ${prefix}${commandName} bs ${page + 1}\n${unisym_js_1.UNIRedux.arrowFromT} Total Pages: ${Math.ceil(sortedItems.length / exports.ITEMS_PER_PAGE)}`;
                return output.replyStyled(result, style);
            },
        },
    ]);
...
exports.style = {
    title: {
        content: `${unisym_js_1.UNISpectra.charm} **G🍓rden**`,
        text_font: "fancy",
        line_bottom: "default",
    },
    contentFont: "fancy",
