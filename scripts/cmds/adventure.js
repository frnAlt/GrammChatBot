...
async function entry(ctx) {
    ...
    const home = new spectral_home_1.SpectralCMDHome({
        ...
        async home({ output, input, usersDB }, { itemList }) {
            ...
            return output.reply(`🌍 | Hello **${cache?.name || "Unregistered"}**! Welcome to ${cassidy_styler_1.FontSystem.applyFonts("Adventure", "double_struck")}!\n` +
                `Please choose one of our **quests**:\n\n` +
                `${unispectra_1.UNIRedux.arrow} ${cassidy_styler_1.FontSystem.applyFonts("All Options", "fancy_italic")}\n` +
                `${itemList.replace(/^/gm, `${unispectra_1.UNIRedux.arrowFromT} `)}\n\n` +
                `📜 | **Status**: ${cache?.adventure?.name
    ...
    }, [
        {
            ...
            async handler() {
                ...
                if (!userData.adventure) {
                    return output.reply(`👤 **${userData.name || "Unregistered"}** (Adventure)\n\nJas register first sir!`);
                }
                let content = [
                    `👤 **${userData.name || "Unregistered"}** (Adventure)`,
                    `${unispectra_1.UNIRedux.standardLine}`,
                    `${unispectra_1.UNIRedux.arrow} ${cassidy_styler_1.FontSystem.applyFonts("Adventure Zones", exports.style.content.text_font)}`,
                ];
                zones.forEach((z) => {
                    ...
                    content.push(`🌍 『 ${z.name} 』`, `**Key:** ${z.key}`, `**Description:** ${z.description}`, `**Cooldown:** ${z.cooldown / 3600000} hours`, `**Status:** ${timeLeft > 0
        ...
        },
        {
            ...
            async handler() {
                ...
                let content = [
                    `👤 **${(await usersDB.getCache(input.senderID))?.name || "Unregistered"}** (Adventure)`,
                    `${unispectra_1.UNIRedux.standardLine}`,
                    `${unispectra_1.UNIRedux.arrow} ${cassidy_styler_1.FontSystem.applyFonts("Adventurer List", exports.style.content.text_font)}`,
                ];
                for (const [userId, userData] of Object.entries(allUsers)) {
                    if (userData.adventure?.name) {
                        ...
                        const items = Object.entries(inventory)
                            ...
                            .join(", ") || "None";
                        content.push(`🌍 『 ${userData.adventure.name} 』`, `**User ID:** ${userId}`, `**Inventory:** ${items}`, `**Coins:** ${userData.money || 0}`, `${unispectra_1.UNIRedux.standardLine}`);
                    }
                }
                if (!content.some((line) => line.includes("『"))) {
                    content.push(`No adventurers registered yet!`);
                }
                content.push(`${unispectra_1.UNIRedux.standardLine}`, ``);
                return output.reply(`\n${content.join("\n")}`);
            },
        },
        {
            ...
            aliases: ["-r"],
            args: ["<name>"],
            validator: new spectral_home_1.CassCheckly([
                { index: 0, type: "string", required: true, name: "name" },
            ]),
            async handler() {
                const name = actionArgs.join("_");
                let userData = await usersDB.getCache(input.senderID);
                userData.adventure ??= {};
                if (userData.adventure?.name) {
                    return output.reply(`👤 **${userData.name || "Unregistered"}** (Adventure)\n${unispectra_1.UNIRedux.standardLine}\n` +
        ...
        },
        {
            ...
            async handler() {
                ...
                if (!zone) {
                    return output.reply(`👤 **${(await usersDB.getCache(input.senderID))?.name || "Unregistered"}** (Adventure)\n${unispectra_1.UNIRedux.standardLine}\n` +
                ...
                let content = [
                    `👤 **${userData.name || "Unregistered"}** (Adventure)`,
                    `${unispectra_1.UNIRedux.standardLine}`,
                    `${unispectra_1.UNIRedux.arrow} ${cassidy_styler_1.FontSystem.applyFonts(`Adventured in ${zone.name}!`, exports.style.content.text_font)}`,
                    `Event: ${outcome.description}`,
                ];
                if (outcome.rewards.coins)
                    content.push(`Earned ${outcome.rewards.coins} coins`);
                if (outcome.rewards.itemKey)
                    content.push(`Found ${outcome.rewards.quantity} ${outcome.rewards.itemKey.replace("_", " ")}`);
                content.push(`Use "${prefix}adventure inventory" to check inventory`, `Use "${prefix}adventure trade" to trade items`, `${unispectra_1.UNIRedux.standardLine}`, ``);
