    ...
    async entry(ctx) {
        ...
        if (subcommand === "cooldown") {
            ...
            if (!userData || !userData.name) {
                return await output.reply("You need to register first! Use: changename <username>");
            }
            const { appleWins = 0, appleCooldown = 0 } = userData;
            const timeNow = Date.now();
            const timeLeft = Math.max(0, Math.ceil((appleCooldown - timeNow) / 1000));
            if (timeLeft > 0) {
                return await output.reply(`Please wait for ${timeLeft} seconds before shooting again.\nTotal Wins: ${appleWins}`);
            }
            return await output.reply(`You can shoot now! Use: appleshot <bet>\nTotal Wins: ${appleWins}`);
        }
        const userData = await usersDB.getItem(userID);
        const bet = (0, ArielUtils_1.parseBet)(args[0], userData.money);
        if (!userData || !userData.name) {
            return await output.reply("You need to register first! Use: changename <username>");
        }
        ...
        if (timeLeft > 0 && !input.isAdmin) {
            return await output.reply(`Please wait for ${Math.ceil(timeLeft / 1000)} seconds before shooting again.\nTotal Wins: ${appleWins}`);
        }
        if (isNaN(bet) || bet < 20) {
            return await output.reply(`Minimum bet is 20 coins. Please enter a valid bet.\n\nTotal Wins: ${appleWins}\n\nExample: appleshot 100`);
        }
        if (money < bet) {
            return await output.reply(`You don't have enough coins to bet ${bet} coins. Current balance: ${(0, ArielUtils_1.formatCash)(money, true)}.`);
        }
        const hitChance = 0.3;
        ...
        if (isHit) {
            ...
            const allUsers = await usersDB.queryItemAll({ "value.name": { $exists: true } }, "name", "userID", "money");
            const userKeys = Object.keys(allUsers).filter((key) => key !== userID && allUsers[key].name);
            if (userKeys.length === 0) {
                resultMessage = [
                    `🏹 **Ouchh...** You missed the apple, but there's no one else to hit!`,
                    `💸 Your bet of ${(0, ArielUtils_1.formatCash)(bet, true)} coins is safe.`,
                ];
            }
            else {
                ...
                resultMessage = [
                    ...
                    `💸 You paid one half of ${(0, ArielUtils_1.formatCash)(bet, true)} coins to **${randomUser.name}** as compensation, but you've lost the other half.`,
                ];
            }
        }
        ...
        resultMessage.push(`💰 **New Balance**: ${(0, ArielUtils_1.formatCash)(finalBalance)} coins.`, `🎯 **Total Wins**: ${newAppleWins}`);
