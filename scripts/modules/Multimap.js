...
class MultiMap {
     ...
     */
    toUnique(predicate) {
        const newMap = new MultiMap();
        const seen = new Set();
        for (const [key, value] of this.#_entries) {
            const identifier = predicate ? predicate(value) : value;
    ...
    getMap(key) {
        ...
        return new MultiMap(entriesForKey);
    }
    /**
     ...
     */
    findAndGetMap(predicate) {
        const matchingEntries = this.#_entries.filter(([key, value]) => predicate(key, value));
        return new MultiMap(matchingEntries);
    }
}
