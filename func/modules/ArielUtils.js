...
exports.ArielIcons = exports.numMultipliers = void 0;
exports.parseBet = parseBet;
exports.abbreviateNumber = abbreviateNumber;
exports.formatCash = formatCash;
exports.formatValue = formatValue;
exports.formatTimeSentence = formatTimeSentence;
exports.formatTimeSentenceV2 = formatTimeSentenceV2;
exports.getMinimumChange = getMinimumChange;
exports.isNoChange = isNoChange;
exports.numMultipliers = {
    "": 1,
    k: 1e3,
    m: 1e6,
    b: 1e9,
...
function parseBet(arg, totalBalance) {
    ...
    const multipliers = exports.numMultipliers;
...
function formatCash(number = 0, emoji = "💵", bold = false) {
    ...
    return `${bold ? "**" : ""}${number > 999 ? `($${abbreviateNumber(number)}) ` : ""}$${Number(number).toLocaleString()}${emoji || "💵"}${bold ? "**" : ""}`;
}
function formatValue(number = 0, emoji = "🎲", bold = false) {
    ...
    return `${bold ? "**" : ""}${number > 999 ? `(${emoji || "🎲"}${abbreviateNumber(number)}) ` : ""}${emoji || "🎲"}${Number(number).toLocaleString()}${bold ? "**" : ""}`;
}
...
function formatTimeSentenceV2(ms, { abbr = false } = {}) {
    const baseUnits = [
    ...
    for (const { label, ms: unitMs } of allUnits) {
        const value = Math.floor(remainingMs / unitMs);
        if (value > 0) {
            parts.push(`${value}${abbr ? "" : " "}${label}${value > 1 && !abbr ? "s" : ""}`);
            remainingMs %= unitMs;
        }
    }
    if (parts.length === 0)
    ...
    if (parts.length === 1)
        return parts[0];
    if (parts.length === 2)
        return `${parts[0]} ${abbr ? "" : "and "}${parts[1]}`;
    return `${parts.slice(0, -1).join(abbr ? " " : ", ")}${abbr ? "" : ","} ${abbr ? "" : "and "}${parts[parts.length - 1]}`;
}
