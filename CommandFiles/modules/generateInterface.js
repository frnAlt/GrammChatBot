...
function generateInterface(name, obj) {
    function parseType(value, indentLevel = 1) {
        if (value === null)
            return "null";
    ...
    function generateInlineObjectType(value, indentLevel) {
        const entries = Object.entries(value);
        if (entries.length === 0)
            return "{}";
        const indent = "  ".repeat(indentLevel);
        const nestedIndent = "  ".repeat(indentLevel + 1);
        const props = entries.map(([key, val]) => `${nestedIndent}${key}: ${parseType(val, indentLevel + 1)};`);
        return `{\n${props.join("\n")}\n${indent}}`;
    }
    const isPlainObject = typeof obj === "object" && !Array.isArray(obj) && obj !== null;
    if (!isPlainObject) {
