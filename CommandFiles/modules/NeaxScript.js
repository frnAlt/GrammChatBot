...
exports.NeaxScript = void 0;
const middleware_1 = require("@root/handlers/middleware/middleware");
...
(function (NeaxScript) {
  ...
  Displays all linked users.`,
    ...
    (function (Codes) {
        ...
        Codes[Codes["MalformedInput"] = 1] = "MalformedInput";
        Codes[Codes["PermissionNeedRise"] = 2] = "PermissionNeedRise";
        Codes[Codes["ExecError"] = 3] = "ExecError";
        Codes[Codes["MissingOrInvalidArgs"] = 4] = "MissingOrInvalidArgs";
        Codes[Codes["CommandNotImplemented"] = 5] = "CommandNotImplemented";
        Codes[Codes["DepError"] = 6] = "DepError";
        Codes[Codes["CommandNotFound"] = 127] = "CommandNotFound";
    })(Codes = NeaxScript.Codes || (NeaxScript.Codes = {}));
    NeaxScript.Commands = {
        async *num({ nsxuCreated, nsxAuthor, usersDB, nsxTarget }) {
            ...
            const num = (0, ArielUtils_1.parseBet)(numx, money);
            yield `${num}`;
            return Codes.Success;
        },
        async *uincr({ usersDB, nsxuCreated, nsxTarget, isOtherTAllowed: isAllowed, nsxAuthor, nsxu, }) {
            if (!nsxTarget) {
        ...
        async *tincr({ threadsDB, nsxuCreated, nsxTarget, isOtherTAllowed: isAllowed, nsxAuthor, nsxu, }) {
            if (!nsxTarget) {
                yield "Missing target.";
                return Codes.MissingOrInvalidArgs;
            }
        ...
        async *ulink({ globalDB, nsxuCreated, nsxTarget, nsxu, isAuthorAdmin, isTargetAdmin, }) {
            if (!isAuthorAdmin) {
                yield nsxu.notAllowed();
                return Codes.PermissionNeedRise;
            }
            ...
            const { links = [] } = await globalDB.getItem(middleware_1.UID_LINKS_KEY);
            ...
            await globalDB.setItem(middleware_1.UID_LINKS_KEY, {
        ...
        async *input({ nsxuCreated, input, nsxu }) {
            ...
            const item = nsxu.getNestedProperty(input, ...nsxuCreated.args);
        ...
        async *arg({ nsxu, nsxTarget, nsxName, nsxuCreated, nsxAuthor, nsxMod, isTargetAdmin, input, isOtherTAllowed: isAllowed, isAuthorAdmin, }) {
            yield nsxu.json({
                nsxName,
        ...
        async *mdemote({ nsxu, nsxTarget, isAuthorAdmin }) {
            ...
            MODERATORBOT.push(nsxTarget);
            Cassidy.config.MODERATORBOT = MODERATORBOT.filter((item) => item !== nsxTarget);
        ...
        async *uget({ usersDB, nsxuCreated, nsxu, nsxTarget, isOtherTAllowed: isAllowed, nsxAuthor, }) {
            if (!nsxTarget) {
                yield "Missing target.";
                return Codes.MissingOrInvalidArgs;
            }
            ...
            if (nsxuCreated.hasFlag("json")) {
                ...
                yield res +
                    (typeof item === "string"
                        ? item
                        : nsxu.inspect(item, nsxuCreated.hasFlag("all")
                            ? 0
                            : parseInt(nsxuCreated.flagValues.get("depth")) ?? undefined));
            }
            return Codes.Success;
        },
        async *tget({ threadsDB, nsxuCreated, nsxu, nsxTarget, isOtherTAllowed: isAllowed, nsxAuthor, }) {
            if (!nsxTarget) {
                yield "Missing target.";
                return Codes.MissingOrInvalidArgs;
            }
    ...
    class Parser {
        ...
        async run(script, callback, full = false) {
            script = String(script);
            script = Parser.parseVariables(this.context, script);
            ...
            const commandFunc = NeaxScript.Commands[commandName];
            let nsxuCreated = NeaxScript.NXSUtil.createFlagsAndArgs(...commandArgs);
            ...
            const commandContext = {
                ...
                nsxTarget,
                nsxMod: mod,
                nsxu: NeaxScript.NXSUtil,
                nsxName: commandName,
                isAuthorAdmin: false,
                isOtherTAllowed(...args) {
                    if (args.length === 0) {
                        return input.senderID === nsxTarget;
                    }
                    return args[0] === args[1];
                },
                riseIsOtherTAllowed(...args) {
                    if (args.length === 0) {
            ...
            try {
                if (mod) {
                    const modFunc = NeaxScript.Modifiers[mod];
                    if (modFunc) {
                ...
                if (full) {
                    ...
                    const result = await this.executeCommand(commandFunc, commandContext, callback);
                    return { result: result.result, code: result.code };
                }
            }
            catch (error) {
                callback(NeaxScript.NXSUtil.err(error));
                console.error(error);
                return { code: Codes.ExecError, result: NeaxScript.NXSUtil.err(error) };
            }
        }
        async runAsync(script) {
            const res = await this.run(script, () => { }, true);
            return {
                ...res,
            };
        }
        async neaxInline(body) {
            body = String(body);
            const reg = /neax\[(.*?)\]/g;
            let result = body;
            const codes = [];
            ...
            for (const [whole, match] of body.matchAll(reg)) {
                const res = await this.runAsync((match + " --raw"));
                if (res.code === 0) {
        ...
        static parseVariables(ctx, script) {
            const reg = /%([^%]+)%/g;
            return String(script).replace(reg, (_, p1) => {
                return `${typeof ctx.input[p1] === "string"
                    ? String(ctx.input[p1])
                    : ctx.input[p1]
                        ? NeaxScript.NXSUtil.json(ctx.input[p1])
                        : "undefined"}`;
            });
        }
        async executeCommand(commandFunc, context, callback) {
            let result = "";
            const iterator = commandFunc(context);
            while (true) {
                const { value, done } = await iterator.next();
                if (done || typeof value !== "string") {
                    return { result, code: value, done };
                }
                result += `${value}\n`;
                try {
                    callback?.(value);
    ...
    NeaxScript.NXSUtil = {
        ...
        err(data) {
            ...
            return `Neax::ProblemData =:\n\n${NeaxScript.NXSUtil.inspect(data)}`;
        },
        notAllowed() {
            return `Neax::PermissionNeedRise`;
        },
        createFlagsAndArgs(...strs) {
            const all = NeaxScript.NXSUtil.flattenArgsAsArray(...strs);
            const flags = all.filter((i) => i.startsWith("--"));
            const args = all.filter((i) => !flags.includes(i));
            const flagValues = flags.reduce((acc, flag, index) => {
                const flagName = flag.replace(/^--/, "");
                const nextValue = all[index + 1] && !all[index + 1].startsWith("--")
                    ? all[index + 1]
                    : true;
        ...
        flattenArgsAsArray(...args) {
            return NeaxScript.NXSUtil.flattenArgs(...args)
                .split(" ")
                .filter(Boolean);
        },
        getNestedProperty(obj, ...keys) {
            const flatKeys = NeaxScript.NXSUtil.flattenArgsAsArray(...keys);
            return flatKeys.reduce((acc, key) => {
...
})(NeaxScript || (exports.NeaxScript = NeaxScript = {}));
