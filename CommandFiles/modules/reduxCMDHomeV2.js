...
exports.ReduxCMDHome = void 0;
// SpectraRework !
const unisym_js_1 = require("./unisym.js");
class ReduxCMDHome {
    configs;
    options;
    constructor({ home, isHypen = false, argIndex = 0, setup = () => { }, entryConfig, entryInfo, }, configs) {
        if (entryConfig) {
    ...
    async runInContext(ctx) {
        ...
        const targets = this.configs.filter((i) => {
            ...
            if (Array.isArray(i.aliases)) {
                return (i.aliases.includes(key) ||
                    i.aliases.some((j) => String(j).toLowerCase() === String(key).toLowerCase() ||
        ...
        if (targets.length > 0) {
            ...
            const itemList = this.createItemLists(this.configs, this.options.isHypen ? ctx.commandName : slicedArgs.join(" "), ctx.prefix);
            // console.log(
            ...
            if (typeof home === "function") {
                ...
                await output.reply(`${unisym_js_1.UNIRedux.burger} **Options**\n\n${itemList}\n\n${unisym_js_1.UNIRedux.standardLine}\nAuto-Generated by ${unisym_js_1.UNIRedux.reduxMark}`);
            }
        }
    }
    createItemList(config, commandName, prefix = global.Cassidy.config.PREFIX) {
        ...
        // );
        return (`${unisym_js_1.UNIRedux.disc} **${prefix}${commandName}${this.options.isHypen ? "-" : " "}${config.key}** [font=fancy_italic]${Array.isArray(config.args) ? config.args.join(" ") : ""}[:font=fancy_italic]${typeof config.description === "string"
            ? `\n${unisym_js_1.UNIRedux.charm} ${config.description}`
            : ""}` +
...
exports.ReduxCMDHome = ReduxCMDHome;
