...
exports.OutputSent = exports.OutputClass = void 0;
const BriefcaseAPI_1 = require("@cass-modules/BriefcaseAPI");
const axios_1 = __importDefault(require("axios"));
const unisym_1 = require("./unisym");
const PageButton_1 = require("./PageButton");
const sendMessage_1 = require("@root/handlers/page/sendMessage");
const webSystem_1 = require("@root/webSystem");
const cassEXP_1 = require("./cassEXP");
const util_1 = require("util");
const styler_1 = require("@cassidy/styler");
const cassidy_styler_1 = require("cassidy-styler");
const InputClass_1 = __importDefault(require("./InputClass"));
class OutputClass {
    #ctx;
    NO_EVENT_MODE;
    /**
    ...
    static createWithoutEvent(api) {
        const ctx = {
            ...
            FontSystem: cassidy_styler_1.FontSystem,
            fonts: cassidy_styler_1.FontSystem.fonts,
            ...
            stylerDummy: new styler_1.CassidyResponseStylerControl({}),
            prefix: Cassidy.config.PREFIX,
        };
        ctx.ctx = ctx;
        ctx.allObj = ctx;
        ctx.input = new InputClass_1.default(ctx);
        const styler = new styler_1.CassidyResponseStylerControl({});
        ctx.styler = styler;
    ...
    constructor(ctx) {
        ...
        this.UIName = null;
        // @ts-ignore
        this.selectItem = BriefcaseAPI_1.BriefcaseAPI.selectItem.bind(ctx);
        const self = this;
    ...
    async req(url, params, configOrMethod = "GET") {
        let finalUrl = url;
        let finalConfig = typeof configOrMethod !== "string"
            ? configOrMethod
        ...
        try {
            const res = await (0, axios_1.default)(finalUrl, finalConfig);
            return res.data;
        }
        catch (error) {
    ...
    async reply(bodyOrListener, callbackRep = (_info) => { }) {
     ...
     */
    error(err, callback = (_info) => { }) {
        let error = err;
        if (typeof error !== "object" && typeof error !== "string") {
            throw new Error(`The first argument must be an Error instance or a string.`);
        }
     ...
     * @returns A promise resolving when the operation is complete.
     */
    add(user, thread = this.#ctx.input?.threadID) {
     ...
     * @returns A promise resolving when the operation is complete.
     */
    kick(user, thread = this.#ctx.input?.threadID) {
    ...
    reaction(emojiOrListener, mid = this.#ctx.input?.messageID) {
     ...
     * @returns A promise resolving to the result of the message.
     */
    async attach(form, stream, style) {
        const body = form;
        try {
            const awaited = typeof stream === "string"
    ...
    syntaxError(commandX) {
        ...
        return this.reply(`❌ The command syntax you are using is invalid, please use ${cmdName ? `${obj.prefix}help ${cmdName}` : `the help command`} to see how it works.`);
    }
    /**
     ...
     * @returns A promise resolving to a boolean indicating success.
     */
    async edit(text, mid, delay, style, options) {
        options ??= {};
        const obj = this.#ctx;
        const { input } = obj;
    ...
    async #processOutput({ ...options }) {
        ...
        const command = cmd;
        if (command?.meta?.noRibbonUI !== true &&
            global.Cassidy.config.noRibbonUI !== true &&
            obj.money &&
            options.noRibbonUI !== true) {
            let hasS = Boolean(input.senderID);
            const { name } = await obj.money.getCache(options.threadID ?? input.senderID);
            ...
            if (options.body && !options.body.trim().startsWith("👤")) {
                options.body =
                    hasS && finalName && finalName !== "Unregistered"
                        ? `👤 **${finalName}**${cmd && !isOther ? ` (${obj.input.words[0]})` : ""}\n\n${options.body}`
                        : `🍃 Register with **${obj.prefix}id-setname** now!\n\n${options.body}`;
            }
        }
        if (command?.meta?.noLevelUI !== true &&
            global.Cassidy.config.noLevelUI !== true &&
            obj.money &&
            options.noLevelUI !== true) {
            let hasS = Boolean(input.senderID);
            const { cassEXP, name } = await obj.money.getCache(options.threadID ?? input.senderID);
            const inst = new cassEXP_1.CassEXP(cassEXP);
            const finalName = this.UIName || name;
            options.body =
                hasS && finalName
                    ? `${options.body}\n${unisym_1.UNIRedux.standardLine}\n${unisym_1.UNIRedux.arrow} ***Level*** ${unisym_1.UNISpectra.nextArrow} ${inst.level} [${inst.exp} / ${inst.getNextEXP()}]`
                    : options.body;
        }
        return options.body;
    }
    async dispatch(body, options = { body: "" }) {
        const text = body;
        const obj = this.#ctx;
        ...
        if (!isStr(options)) {
            ...
            if (Cassidy.config.autoGoogleTranslate) {
                try {
                    options.body = (await (0, unisym_1.translate)(options.body, Cassidy.config.autoGoogleTranslate))?.text;
                }
                catch (error) {
                    console.error(error);
                }
            }
            if (PageButton_1.PagePayload.isPageButton(options.attachment) && !obj.input.isPage) {
                // @ts-ignore
                const buttons = PageButton_1.PagePayload.fromPayload(options.attachment);
                options.body = buttons.toString();
            }
            if (PageButton_1.PagePayload.isPageButton(options.attachment)) {
                delete options.body;
            }
            ...
            const stylerShallow = STYLE
                ? obj.stylerDummy.shallowMake(Object.assign({}, options.defStyle ?? {}, input.defStyle ?? {}), Object.assign({}, options.style ?? {}, input.style ?? {}, STYLE ?? {}))
                : styler.shallowMake(Object.assign({}, options.defStyle ?? {}, input.defStyle ?? {}), Object.assign({}, options.style ?? {}, input.style ?? {}));
            if (options.body) {
                options.body = await this.#processOutput(options);
            }
            if (!options.noStyle && options.body) {
                options.body = unisym_1.UNISpectra.standardizeLines(options.body);
            ...
            if (input.isWeb) {
                let url = null;
                if (typeof options.attachment === "object" &&
                    options.attachment &&
                    "_readableState" in options.attachment) {
                    const temp = new sendMessage_1.TempFile();
                    const { fileTypeFromBuffer } = await global.fileTypePromise;
                    const base64_ = await (0, webSystem_1.streamToBase64)(options.attachment);
                    const buffer = Buffer.from(base64_, "base64");
                    const type = await fileTypeFromBuffer(buffer);
                    let format = type?.ext;
                    const newStream = (0, webSystem_1.base64ToStream)(base64_);
            ...
            const api = optionsCopy.useWebMode || optionsCopy.threadID === "webpanel"
                ? globalThis.wssAPI
                : this.api;
            return new Promise(async (res) => {
                if (optionsCopy.contactID && input.isFacebook) {
                    this.api.shareContact(options.body, optionsCopy.contactID, optionsCopy.threadID || event.threadID);
                    res(
                    // @ts-ignore
                    new OutputSent(obj, {
                        ...options,
                ...
                if (api.sendMessage?.length === 5) {
                    api.sendMessage(
                    // @ts-ignore
                    options, optionsCopy.threadID || event.threadID, cbb, optionsCopy.messageID ||
                        (optionsCopy.isReply ? event.messageID : null));
                }
                else {
                    const res = await api.sendMessage(
                    // @ts-ignore
                    options, optionsCopy.threadID || event.threadID, optionsCopy.messageID ||
                        (optionsCopy.isReply ? event.messageID : null));
                    cbb(null, res);
                }
            });
    ...
    formatError(error) {
        ...
        if (error instanceof Error) {
            ...
            errorMessage += (0, util_1.inspect)(error, { depth: null, showHidden: true });
        }
        return errorMessage;
    }
    /**
     ...
     */
    async confirm(body, done, extraStyle) {
        const text = `⚠️ ${body}\n${unisym_1.UNIRedux.standardLine}\n**Yes** | **No**`;
            ...
            input.setReply(info.messageID, {
                ...
                callback(repCtx) {
                    ...
                    if (!newCtx.yes && !newCtx.no) {
                        return repCtx.output.reply(`❌ Invalid response, please go back and reply either **yes** or **no**.`);
                    }
                    done?.(newCtx);
                    resolve(newCtx);
...
exports.OutputSent = OutputSent;
exports.default = OutputClass;
