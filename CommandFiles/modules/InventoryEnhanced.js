...
exports.Collectibles = exports.Inventory = void 0;
...
class Inventory {
    ...
    sanitize(inv = this.inv) {
        ...
        let result = inv.map((item, index) => {
            ...
            const { name = "Unknown Item", key = "", flavorText = "Mysteriously not known to anyone.", icon = "❓", type = "generic", cannotToss = false, sellPrice = 0, uuid = Inventory.generateUUID(), } = item;
            if (!key) {
                return;
            }
            let result = {
    ...
    deleteRef(item) {
        ...
        let index = index1 === -1 ? this.inv.indexOf(this.getOneByID(item.uuid)) : index1;
        if (index !== -1) {
     ...
     */
    toUnique(callback) {
        return Datum_1.Datum.toUniqueArray(this.inv, callback ?? ((i) => i.key));
    }
    /**
     ...
     */
    toUniqueInventory(callback) {
        return new Inventory(this.toUnique(callback));
    }
    /**
    ...
    merge(other) {
        ...
        if (this.limit && combined.length > this.limit) {
            throw new Error(`Cannot merge: exceeds inventory limit of ${this.limit}.`);
        }
    ...
    findByName(name, caseSensitive = false) {
        ...
        return this.inv.filter((item) => caseSensitive
    ...
    validate() {
        ...
        this.inv.forEach((item, index) => {
            if (!item.key)
                errors.push(`Item at index ${index} missing key.`);
            if (!item.name)
     ...
     */
    toMultiMap() {
        return new Multimap_1.MultiMap(this.toEntries());
    }
    /**
...
class Collectibles {
    ...
    constructor(collectibles = []) {
        ...
        try {
            const data = JSON.parse((0, fs_1.readFileSync)(process.cwd() +
                "/CommandFiles/resources/collectibles/collectibles.json", "utf-8"));
    ...
    sanitize(c = this.#collectibles) {
        ...
        for (let i = c.length - 1; i >= 0; i--) {
            ...
            if (!collectible.metadata)
                continue;
            let { key, name = "Unknown Collectible", icon = "❓", type = "generic", limit = undefined, } = collectible.metadata;
            if (!key)
                continue;
    ...
    *keys() {
    ...
    set(key, amount) {
        ...
        if (index !== -1) {
            ...
            if (this.#collectibles[index].metadata.limit) {
                this.#collectibles[index].amount = Math.min(this.#collectibles[index].amount, this.#collectibles[index].metadata.limit);
            }
        }
        return index;
    }
    ...
    atLimit(key) {
        const data = this.get(key);
        return (data?.metadata.limit !== undefined && data.amount >= data.metadata.limit);
    }
    validate(key) {
        if (!this.get(key)) {
    ...
    remove(key) {
        this.validate(key);
        this.#collectibles = this.#collectibles.filter((c) => c?.metadata.key !== key);
    }
    removeEmpty() {
