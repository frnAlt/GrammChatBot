...
exports.GearsManage = exports.GearData = exports.PetTurns = exports.PetPlayer = exports.WildPlayer = exports.PetGame = exports.spells = exports.petSpellMap = exports.ElementalChild = exports.ElementalChilds = exports.elementalPets = exports.elementalMapping = exports.meta = void 0;
exports.randArr = randArr;
exports.randObj = randObj;
exports.use = use;
const ArielUtils_1 = require("@cass-modules/ArielUtils");
const Numero_1 = require("@cass-modules/Numero");
...
class ElementalChilds {
    ...
    getAllStrongs() {
        return this.elements.reduce((acc, i) => [...acc, ...Object.keys(i.strong)], []);
    }
    getAllWeaks() {
        return this.elements.reduce((acc, i) => [...acc, ...Object.keys(i.weak)], []);
    }
    ...
    getGapPets() {
        let result = [];
        for (const key in exports.elementalPets) {
            const value = exports.elementalPets[key];
            const childs = new ElementalChilds(...value);
            const acc = Math.abs(childs.getModifierAgainst(this) - this.getModifierAgainst(childs));
            if (acc === 0) {
                continue;
            }
...
exports.ElementalChilds = ElementalChilds;
class ElementalChild {
    element;
    constructor(element, mapping = exports.elementalMapping) {
        if (element instanceof ElementalChild) {
            // @ts-ignore
            element = element.element;
        }
        this.element = JSON.parse(JSON.stringify({
            // @ts-ignore
...
exports.ElementalChild = ElementalChild;
exports.petSpellMap = {
...
exports.spells = {
    ferocious_bark: {
        ...
        flavorText: "A bark that intimidates your opponent, reducing their attack power temporarily for 3 turns.",
        type: "opp_change",
    ...
    loyal_guard: {
        ...
        flavorText: "Grants a protective buff to allies, increasing their defense against enemy attacks for 3 turns.",
    ...
    shadow_pounce: {
        ...
        flavorText: "Allows the pet to swiftly attack from the shadows, dealing surprise damage.",
        type: "opp_attack",
    ...
    nimble_dodge: {
        ...
        flavorText: "Enables the pet to evade attacks more effectively for a short duration of 2 turns.",
    ...
    antler_strike: {
        ...
        flavorText: "Charges forward with antlers, weakening enemies in its path for 2 turns.",
        type: "opp_change",
        value(player, opponent) {
            const magic = player.MAGIC;
            const oppAtk = opponent.ATK;
    ...
    flame_of_rejuvenation: {
        ...
        tp: 22, // Reduced TP cost
        flavorText: "Surrounds allies in healing flames, restoring health over time.",
        type: "ally_heal",
...
class PetGame {
    ...
    static useSpell(spellKey, _caster, _target) {
        const spell = exports.spells[spellKey];
...
class WildPlayer {
    ...
    #mercy;
    getPlayerUI({ turn = false, pop = null, icon = null, upperPop = null, selectionOptions = undefined, damageTemp = 0, } = {}) {
        ...
        txt += `**HP**: ${fled ? "**" : ""}${newHP}/${this.maxHP}${fled ? "** ⚠️" : ""} ${pop ? `(${pop})` : `(${Math.floor((newHP / this.maxHP) * 100)}%)`}\n**MERCY**: ${this.MERCY}%`;
        if (turn) {
    ...
    getAttackMenu() {
        let result = "";
        const [answer = "...", attackName = "Triple dots attack."] = randObj(this.attacks ?? {});
    ...
    getAct(act) {
        ...
        let { pet: selector, flavor = `You performed ${act}!`, response: responseArr = [this.getNeutralDialogue()], petLine: petLineArr = [`Hi I performed ${act}!`], mercyPts = 1, } = targetAct;
        ...
        function helper(text) {
            return text.replaceAll("{name}", `${targetPet.petIcon} **${targetPet.petName}**`);
        }
        return {
            flavor,
            selector,
...
class PetPlayer {
    ...
    getElementals() {
        const mapping = exports.elementalPets[this.petType];
        const childs = new ElementalChilds(...mapping);
        return childs;
    }
    getPlayerUI({ turn = false, pop = null, icon = null, upperPop = null, showStats = false, selectionOptions = undefined, hideHP = false, } = {}) {
        let txt = `${icon ?? this.petIcon} **${this.petName} LV${this.level}** ${!upperPop ? (this.isDown() ? `(***DOWN***)` : ``) : `(***${upperPop}***)`}\n`;
        txt += `**HP**: ${hideHP ? "??" : this.HP}/${hideHP ? "??" : this.maxHP} ${pop
            ? `(${pop})`
            : `(${hideHP ? "??" : Math.floor((this.HP / this.maxHP) * 100)}%)`}`;
        if (showStats) {
            const stat = this.calculateDamageReduction();
            txt += `\n ⚔️ **${(0, ArielUtils_1.abbreviateNumber)(this.ATK)}** | 🔰 **${(0, ArielUtils_1.abbreviateNumber)(this.DF)}** (${(0, ArielUtils_1.abbreviateNumber)(stat.defReduction)}/${(0, ArielUtils_1.abbreviateNumber)(stat.per)}) | 🔥 **${(0, ArielUtils_1.abbreviateNumber)(this.MAGIC)}**`;
        }
        if (turn) {
            txt += `\n\n${this.getSelectionUI(selectionOptions)}`;
        }
    ...
    debug() {
        return (global.utils.representObject(this) +
    ...
    get DF() {
        ...
        return (armorDfs +
            Math.floor(Numero_1.Numero.statDiminishingPower(weaponDf, 80)) +
            extra +
            this.defModifier);
    }
    get DF_OLD() {
    ...
    get ATK() {
        ...
        return Math.round(Math.floor(Numero_1.Numero.statDiminishingPower(armorAtks, 80)) +
            weaponAtks +
            extra +
            this.atkModifier);
    }
    ...
    calculateAttackOld(enemyDef) {
        const atk = this.ATK;
        const df = enemyDef;
        return Math.max(1, Math.floor((atk - df / 5 + Math.floor(Math.random() * 8)) * (1 - df / 100) * 2.2));
    }
    calculateAttack2(enemyDef, atk = this.ATK) {
        atk ??= this.ATK;
        const df = enemyDef;
        return Math.max(1, this.calculateReducedDamage(Math.floor((atk + Math.floor(Math.random() * 15)) * 2.2), df));
    }
    calculateAttack(enemyDef, atk = this.ATK) {
        ...
        atk ??= this.ATK;
        const df = enemyDef;
        const effectiveAtk = Numero_1.Numero.statDiminishingPower(atk, 89);
        return Math.max(1, this.calculateReducedDamage(Math.floor(Numero_1.Numero.applyVariance(effectiveAtk, 0.1) * 2.2), df));
    }
    calculateAttackLinear(enemyDef, atk = this.ATK) {
        ...
        const df = enemyDef;
        const effectiveAtk = Numero_1.Numero.statDiminishingPower(atk, 89);
        return Math.max(1, this.calculateReducedDamageLinear(Math.floor(Numero_1.Numero.applyVariance(effectiveAtk, 0.1) * 2.2), df));
    }
    /**
     ...
     */
    calculateReducedDamage2(damage, def = this.DF, factor_ = 0) {
        const k = 5;
     ...
     * @returns {number} - The reduced damage
     */
    calculateReducedDamage(damage, def = this.DF) {
    ...
    calculateDamageReduction(def = this.DF) {
        const k = 5;
        const defReduction = Numero_1.Numero.statDiminishingPower(def / 5, 85);
        return {
    ...
    get gearMAGIC() {
        const gearMagic = 0;
        const cappedGearMagic = Numero_1.Numero.statDiminishingPower(gearMagic, 60);
    ...
    get MAGIC() {
        const gearMagic = this.weapon.reduce((acc, weapon) => acc + weapon.magic, 0) +
    ...
    static getExtraMagicOf(magic, lastExp) {
        ...
        return Math.floor(magic + Numero_1.Numero.statDiminishingPower(expFactor, 60));
    }
    static getExtraMagicOfOLD(magic, lastExp) {
    ...
    static get petSpellMap() {
        return exports.petSpellMap;
    }
}
exports.PetPlayer = PetPlayer;
var PetTurns;
(function (PetTurns) {
    ...
    PetTurns.getProfile = getProfile;
    function Bash({ activePet, targetPet, opponentStats, petStats, prevMove = "", dodgeChance = Math.random(), }) {
        let flavor = `${unispectra_1.UNIRedux.charm} ${activePet.petIcon} **${activePet.petName}** used 🥊 **Bash**!\n`;
        if ((prevMove === "bash" && dodgeChance < 0.7) || dodgeChance < 0.1) {
            flavor += `${unispectra_1.UNIRedux.charm} ${getProfile(targetPet)} dodged!`;
        ...
        flavor += `${unispectra_1.UNIRedux.charm} Dealt **${damage}** physical damage.\n${targetPet.getPlayerUI()}`;
    ...
    PetTurns.Bash = Bash;
    function HexSmash({ activePet, targetPet, petStats, opponentStats, prevMove = "", dodgeChance = Math.random(), }) {
        let flavor = `${unispectra_1.UNIRedux.charm} ${activePet.petIcon} **${activePet.petName}** used 💥 **HexSmash**!\n`;
        ...
        let damage = Math.round(activePet.calculateAttack(targetPet.DF, meanStat) * 1.5);
        let damageOrig = damage;
        damage = Math.min(damage, Math.round(targetPet.maxHP * 0.5));
        targetPet.HP -= damage;
        petStats.totalDamageDealt += damage;
        opponentStats.totalDamageTaken += damage;
        flavor += `${unispectra_1.UNIRedux.charm} Dealt **${damage}** magical damage.\n${targetPet.getPlayerUI()}`;
        if (damageOrig !== damage) {
            flavor += `\n(The damage has been capped from **${damageOrig}** to **${damage}**)`;
        }
        return {
    ...
    PetTurns.HexSmash = HexSmash;
    function FluxStrike({ activePet, targetPet, prevMove = "", petStats, opponentStats, dodgeChance = Math.random(), }) {
        let flavor = `${unispectra_1.UNIRedux.charm} ${activePet.petIcon} **${activePet.petName}** used 🌩️ **FluxStrike**!\n`;
    ...
    PetTurns.FluxStrike = FluxStrike;
    function ChaosBolt({ activePet, targetPet, prevMove = "", petStats, opponentStats, dodgeChance = Math.random(), }) {
        let flavor = `${unispectra_1.UNIRedux.charm} ${activePet.petIcon} **${activePet.petName}** used ⚡ **ChaosBolt**!\n`;
        ...
        let damage = Math.round((activePet.calculateAttack(targetPet.DF, effectiveStat) * statFactor) **
            1.01);
        const baseChaosChance = Math.min(((activePet.ATK + activePet.MAGIC) / (targetPet.DF || 1)) * 0.2, 0.3);
        const critChance = Math.max(0.1, baseChaosChance / (1 + petStats.attackBoosts));
        if (Math.random() < critChance && statFactor >= 1) {
            ...
            flavor += `${unispectra_1.UNIRedux.charm} 🌪️ **Critical Chaos Hit!**,  **${activePet.petName}** is empowered (+**${boost}** ATK).\n`;
        }
        targetPet.HP -= damage;
        petStats.totalDamageDealt += damage;
        opponentStats.totalDamageTaken += damage;
    ...
    PetTurns.ChaosBolt = ChaosBolt;
    function LastStand({ activePet, targetPet, petStats, opponentStats, dodgeChance = Math.random(), }) {
        let flavor = `${unispectra_1.UNIRedux.charm} ${activePet.petIcon} **${activePet.petName}** tried 🛡️ **Last Stand**!\n`;
        const failChance = 0.6;
        if (dodgeChance < failChance) {
            flavor += `${unispectra_1.UNIRedux.charm} The move failed! ${activePet.petName} couldn't pull it off...`;
        ...
        petStats.totalDamageDealt += damage;
        opponentStats.totalDamageTaken += damage;
        flavor += `${unispectra_1.UNIRedux.charm} Dealt **${damage}** damage, empowered by ${activePet.petName}'s resilience!\n${targetPet.getPlayerUI()}`;
        return {
            damage,
            dodged: false,
            flavor,
    ...
    PetTurns.LastStand = LastStand;
})(PetTurns || (exports.PetTurns = PetTurns = {}));
class GearData {
    ...
    get weapon() {
        const i = PetPlayer.sanitizeWeapon((this.weaponArray ?? []).filter(Boolean)).filter(Boolean);
        this.weaponArray = i;
        return i;
    }
    get armors() {
        const i = PetPlayer.sanitizeArmors((this.armorsArray ?? []).filter(Boolean)).filter(Boolean);
        this.armorsArray = i;
        return i;
    }
    getWeaponUI(pad = "") {
        ...
        if (this.weapon.length === 0) {
            ...
            re = this.weapon
                .map((weapon) => `${pad ? `${pad} ` : ""} ${weapon.icon} **${weapon.name}**\nATK ${weapon.atk} DEF ${weapon.def} MAGIC ${weapon.magic}`)
                .join("\n");
        }
        if (this.weapon.length < GearData.MAX_WEAPON_SLOTS) {
            re += `\n[ ...Free ${GearData.MAX_WEAPON_SLOTS - this.weapon.length} weapon slots. ]`;
        }
        return re;
    }
    hasGear() {
    ...
    getArmorsUI(pad = "") {
        ...
        if (this.armors.length < GearData.MAX_ARMOR_SLOTS) {
            res += `\n[ ...Free ${GearData.MAX_ARMOR_SLOTS - this.armors.length} armor slots. ]`;
        }
        return res;
    }
    toJSON() {
...
class GearsManage {
    ...
    getGearData(key) {
        return (this.gearsData.find((i) => i.key === key) ??
            new GearData({ key }));
    }
...
exports.GearsManage = GearsManage;
// export class Quest {
//   data: any;
...
async function use(obj) {
    ...
    obj.elementalMapping = exports.elementalMapping;
    obj.ElementalChild = ElementalChild;
    obj.ElementalChilds = ElementalChilds;
    obj.elementalPets = exports.elementalPets;
    obj.next();
}
